...
class Validator
{
	...
	function load($xml_path)
	{
		...
		$output = Zittme\Framework\Parsers\RulesetParser::loadXML($xml_path);
		if (!$output)
		{
			return false;
		}
	...
	function getJsPath()
	{
		...
		Zittme\Framework\Storage::write($filepath, $content);
	...
	protected static function _execExpression($value, $expression)
	{
		...
		if (!Zittme\Framework\Storage::exists($filename))
		{
