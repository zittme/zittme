...
class FileHandler
{
	...
	{
		return Zittme\Framework\Storage::copyDirectory(self::getRealPath($source_dir), self::getRealPath($target_dir), $filter ?: '');
	}
	...
	public static function copyFile($source, $target, $force = 'Y')
	{
		...
		return Zittme\Framework\Storage::copy(self::getRealPath($source), self::getRealPath($target));
	}
	...
	{
		return Zittme\Framework\Storage::read(self::getRealPath($filename));
	}
	...
	public static function writeFile($filename, $buff, $mode = "w")
	{
		...
		return Zittme\Framework\Storage::write(self::getRealPath($filename), (string)$buff, (string)$mode);
	}
	...
	public static function removeFile($filename)
	{
		...
		return Zittme\Framework\Storage::delete(self::getRealPath($filename));
	}
	...
	public static function rename($source, $target)
	{
		...
		return Zittme\Framework\Storage::move(self::getRealPath($source), self::getRealPath($target));
	}
	...
	public static function moveDir($source_dir, $target_dir)
	{
		...
		return Zittme\Framework\Storage::move(self::getRealPath($source_dir), self::getRealPath($target_dir));
	}
	...
	{
		$list = Zittme\Framework\Storage::readDirectory(self::getRealPath($path), (bool)$concat_prefix, true, false);
		if (!$list)
		{
			return array();
		}
	...
	public static function makeDir($path_string)
	{
		...
		return Zittme\Framework\Storage::isDirectory($path) || Zittme\Framework\Storage::createDirectory($path);
	}
	...
	public static function removeDir($path)
	{
		...
		return Zittme\Framework\Storage::deleteDirectory(self::getRealPath($path));
	}
	...
	public static function removeBlankDir($path)
	{
		...
		return Zittme\Framework\Storage::deleteEmptyDirectory(self::getRealPath($path), false);
	}
	...
	public static function removeFilesInDir($path)
	{
		...
		return Zittme\Framework\Storage::deleteDirectory(self::getRealPath($path), false);
	}
	...
	public static function getRemoteResource($url, $body = null, $timeout = 3, $method = 'GET', $content_type = null, $headers = array(), $cookies = array(), $post_data = array(), $settings = array())
	{
		...
		$response = \Zittme\Framework\HTTP::request($url, strval($method),
			$body ?: $post_data,
			$converted_headers,
			$converted_cookies,
			$converted_settings
	...
	public static function getRemoteFile($url, $target_filename, $body = null, $timeout = 3, $method = 'GET', $content_type = null, $headers = array(), $cookies = array(), $post_data = array(), $request_config = array())
	{
		...
		if (!Zittme\Framework\Storage::isDirectory($target_dirname) && !Zittme\Framework\Storage::createDirectory($target_dirname))
		{
			return false;
		}
	...
	public static function createImageFile($source_file, $target_file, $resize_width = 0, $resize_height = 0, $target_type = '', $thumbnail_type = 'fill', $quality = 100, $rotate = 0)
	{
		...
		elseif ($type === 'webp' && function_exists('imagecreatefromwebp') && !Zittme\Framework\Image::isAnimatedWebP($source_file))
		{
		...
		@chmod($target_file, 0666 & ~Zittme\Framework\Storage::getUmask());
	...
	public static function readIniFile($filename)
	{
		...
		if(!Zittme\Framework\Storage::isReadable($filename))
		{
			return false;
		}
	...
	public static function openFile($filename, $mode)
	{
		...
		Zittme\Framework\Storage::createDirectory(dirname($filename));
	...
	{
		return Zittme\Framework\Storage::getSize(self::getRealPath($filename)) > 0;
	}
	...
	{
		$filename = self::getRealPath($filename);
		return Zittme\Framework\Storage::exists($filename) ? $filename : false;
	}
	...
	public static function isDir($path)
	{
		...
		return Zittme\Framework\Storage::isDirectory($path) ? $path : false;
	}
	...
	{
		$path = self::getRealPath($path);
		return Zittme\Framework\Storage::isDirectory($path) && Zittme\Framework\Storage::isWritable($path);
	}
