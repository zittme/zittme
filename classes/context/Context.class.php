...
class Context
{
	...
	public static function init()
	{
		...
		if (in_array(self::$_instance->request_method, self::$_router_methods))
		{
			...
			$request = Zittme\Framework\Router::parseURL($method, RX_REQUEST_URL, Zittme\Framework\Router::getRewriteLevel());
			self::$_current_request = $request;
			self::setRequestArguments($request->args);
		}
		else
		{
			self::$_current_request = new Zittme\Framework\Request;
			self::setRequestArguments();
		}
		self::setUploadInfo();

		// If Zittme is installed, get virtual site information.
		if(self::isInstalled())
		{
			if (PHP_SAPI === 'cli')
			{
		...
		else
		{
			...
			$site_module_info->domain = Zittme\Framework\URL::getCurrentDomain();
		...
		{
			$ua_locale = Zittme\Framework\UA::getLocale();
		...
		if ($set_lang_cookie)
		{
			Zittme\Framework\Cookie::set('lang_type', $lang_type, ['expires' => 365, 'path' => \RX_BASEURL]);
		}
		...
		$lang = Zittme\Framework\Lang::getInstance($lang_type);
		...
		else
		{
			if (self::$_current_request->getRouteOption('enable_session'))
			{
				...
				Zittme\Framework\Session::start(false);
			}
		...
		if (self::isInstalled())
		{
			if (Zittme\Framework\Session::getMemberSrl())
			{
		...
		// start debugging
		Zittme\Framework\Debug::isEnabledForCurrentUser();
		...
		{
			$current_url = Zittme\Framework\URL::decodeIdna($current_url);
		}
		if (strpos($request_uri, 'xn--') !== false)
		{
			$request_uri = Zittme\Framework\URL::decodeIdna($request_uri);
		}
	...
	{
		return Zittme\Framework\Session::isStarted();
	}
	...
	{
		return Zittme\Framework\Session::checkStart($force);
	}
	...
	public static function close()
	{
		...
		if (Zittme\Framework\Session::checkStart())
		{
	...
	public static function loadDBInfo($config = null)
	{
		...
		if ($config === null)
		{
			$config = Zittme\Framework\Config::getAll();
		}
	 ...
	 *
	 * @deprecated
	 * @return Zittme\Framework\Request
	 */
	public static function getRouteInfo(): Zittme\Framework\Request
	{
	 ...
	 * @return Zittme\Framework\Request
	 */
	public static function getCurrentRequest(): Zittme\Framework\Request
	{
		return self::$_current_request;
	}
	...
	{
		$list = Zittme\Framework\Lang::getSupportedList();
	...
	public static function loadLangSelected()
	{
		...
		{
			$supported = Zittme\Framework\Lang::getSupportedList();
			$selected = Zittme\Framework\Config::get('locale.enabled_lang');
			if ($selected)
			{
	...
	public static function loadLang($path, $plugin_name = null)
	{
		...
		if (!(($GLOBALS['lang'] ?? null) instanceof Zittme\Framework\Lang))
		{
			$GLOBALS['lang'] = Zittme\Framework\Lang::getInstance(self::$_instance->lang_type ?: config('locale.default_lang') ?: 'ko');
	...
	{
		if (!$GLOBALS['lang'] instanceof Zittme\Framework\Lang)
		{
			$GLOBALS['lang'] = Zittme\Framework\Lang::getInstance(self::$_instance->lang_type ?: config('locale.default_lang') ?: 'ko');
			$GLOBALS['lang']->loadDirectory(RX_BASEDIR . 'common/lang', 'common');
		}
	...
	public static function replaceUserLang($string, $fix_double_escape = false)
	{
		...
		if($lang === null)
		{
			$lang = Zittme\Framework\Cache::get('site_and_module:user_defined_langs:0:' . self::getLangType());
			if($lang === null)
			{
	...
	{
		return Zittme\Framework\URL::encodeIdna((string)$domain);
	}
	...
	{
		return Zittme\Framework\URL::decodeIdna((string)$domain);
	}
	...
	public static function setRequestArguments(array $router_args = [])
	{
		...
		if(isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] === 'POST' && empty($_POST) && !empty($GLOBALS['HTTP_RAW_POST_DATA']))
		{
			...
			{
				if(!Zittme\Framework\Security::checkXXE($GLOBALS['HTTP_RAW_POST_DATA']))
				{
					self::$_instance->security_check = 'DENY ALL';
				...
				$params = Zittme\Framework\Parsers\XMLRPCParser::parse($GLOBALS['HTTP_RAW_POST_DATA']) ?: [];
			}
	...
	public static function setUploadInfo()
	{
		...
		foreach ($_FILES as $key => $val)
		{
			...
			if(!is_array($tmp_name))
			{
				...
				$val['name'] = Zittme\Framework\Filters\FilenameFilter::clean($val['name']);
			...
			else
			{
				...
				foreach ($tmp_name as $i => $j)
				{
					...
					$file = array();
					$file['name'] = Zittme\Framework\Filters\FilenameFilter::clean($val['name'][$i]);
	...
	private static function _filterRequestVar($key, $val)
	{
		...
		foreach($val as $_key => $_val)
		{
			...
			elseif($_val = trim($_val))
			{
				...
				{
					if (!Zittme\Framework\URL::isInternalURL($_val))
					{
						self::$_instance->security_check = 'DENY ALL';
						self::$_instance->security_check_detail = 'ERR_UNSAFE_VAR';
						$_val = null;
	...
	private static function enforceSiteLock()
	{
		...
		if (Zittme\Framework\Filters\IpFilter::inRanges(RX_CLIENT_IP, config('lock.allow') ?: []))
		{
			return;
		}
		...
		$site_module_info = self::get('site_module_info');
		if (Zittme\Framework\URL::getCurrentDomain() !== $site_module_info->domain)
		{
			$url = ($site_module_info->security === 'always') ? 'https://' : 'http://';
			$url = $url . Zittme\Framework\URL::encodeIdna($site_module_info->domain);
	...
	{
		// Change current directory to the Zittme installation path.
		chdir(\RX_BASEDIR);
	...
	public static function getUrl($num_args = 0, $args_list = array(), $domain = null, $encode = TRUE, $autoEncode = FALSE)
	{
		...
		{
			$rewrite_level = Zittme\Framework\Router::getRewriteLevel();
		}
		...
		{
			$current_domain = parse_url(Zittme\Framework\URL::getCurrentDomainURL(), PHP_URL_HOST);
		}
		...
		if(count($get_vars) > 0)
		{
			$query = Zittme\Framework\Router::getURL($get_vars, $force_rewrite_level ?: $rewrite_level);
		}
	...
	{
		return RX_BASEDIR . Zittme\Framework\Config::OLD_DB_CONFIG_PATH;
	}
	...
	{
		return RX_BASEDIR . Zittme\Framework\Config::OLD_FTP_CONFIG_PATH;
	}
	...
	{
		return Zittme\Framework\Router::getRewriteLevel();
	}
