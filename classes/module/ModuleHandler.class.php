...
class ModuleHandler extends Handler
{
	...
	public function procModule()
	{
		...
		{
			if(isset($xml_info->action->{$this->act}) && $xml_info->action->{$this->act}->check_csrf !== 'false' && !Zittme\Framework\Security::checkCSRF())
			{
		...
		// Responsive view on a narrow screen: hand the skin the mobile header and
		// footer under the names it already renders. Only mobile skins (m.skins/*)
		// ever read mobile_header_text, and the responsive view keeps the PC skin,
		// so swapping here is what makes those settings reachable at all — and it
		// works with existing third-party skins, which need no change.
		if(Mobile::isResponsiveView($this->module_info) && Mobile::isNarrowScreen())
		{
			if(!empty($this->module_info->mobile_header_text))
			{
				$this->module_info->header_text = $this->module_info->mobile_header_text;
			}
			if(!empty($this->module_info->mobile_footer_text))
			{
				$this->module_info->footer_text = $this->module_info->mobile_footer_text;
			}
		}
		...
		if($class_name)
		{
			...
			{
				$class_fullname = sprintf('Zittme\\Modules\\%s\\%s', $this->module, $class_name);
			}
		...
		if(!isset($xml_info->action->{$this->act}) || !method_exists($oModule, $this->act))
		{
			...
			if(!empty($forward->module))
			{
				...
				if(!in_array(Context::getRequestMethod(), self::$_nocsrf_methods) && Context::isInstalled())
				{
					if($xml_info->action->{$this->act} && $xml_info->action->{$this->act}->check_csrf !== 'false' && !Zittme\Framework\Security::checkCSRF())
					{
						return self::_createErrorMessage(-1, 'msg_security_violation', 403, 'ERR_CSRF_CHECK_FAILED');
					}
				}
				...
				if(!empty($forward->class_name))
				{
					...
					{
						$class_fullname = sprintf('Zittme\\Modules\\%s\\%s', $forward->module, $forward->class_name);
					}
				...
				{
					Zittme\Modules\Admin\Controllers\Base::getInstance()->loadAdminMenu($forward->module);
	...
	protected function _setModuleColorScheme($site_module_info)
	{
		...
		{
			$color_scheme = Zittme\Framework\UA::getColorScheme();
		}
						...
						{							// No menu selected
							...
							else
							{
								if($menu->menu_srl == -1)
								{
									...
									// Cache may be missing after a full cache clear; regenerate from the site start module.
									if(!$homeMenuSrl)
									{
										$start_module = getModel('module')->getSiteInfo(0);
										if($start_module && !empty($start_module->mid))
										{
											$tmpArgs = new stdClass;
											$tmpArgs->url = $start_module->mid;
											$tmpArgs->site_srl = 0;
											$output = executeQuery('menu.getMenuItemByUrl', $tmpArgs);
											if($output->toBool() && !empty($output->data->menu_srl))
											{
												$homeMenuSrl = $output->data->menu_srl;
	...
	public static function triggerCall($trigger_name, $called_position, &$obj)
	{
		...
		foreach($triggers as $item)
		{
			...
			{
				$class_name = ($type[0] === '\\') ? $type : sprintf('Zittme\\Modules\\%s\\%s', $module, $type);
			...
			catch (Zittme\Framework\Exception $e)
			{
			...
			if ($trigger_name !== 'common.flushDebugInfo')
			{
				...
				{
					Zittme\Framework\Debug::addTrigger(array(
