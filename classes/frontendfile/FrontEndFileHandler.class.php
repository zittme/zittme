...
class FrontEndFileHandler extends Handler
{
	...
	protected function getFileInfo($fileName, $unused = '', $media = 'all', $vars = array(), $isCommon = false)
	{
		...
		// Since Zittme 2.1, targetIe is always empty
		$file->targetIe = '';
	...
	protected function proc_CSS_JS($file, $minify)
	{
		...
		if (!file_exists($minifiedFilePath) || filemtime($minifiedFilePath) < filemtime($file->fileFullPath))
		{
			...
			$success = Zittme\Framework\Formatter::$method_name($file->fileFullPath, $minifiedFilePath);
			if ($success === false)
			{
				return;
			}
	...
	protected function proc_LESS_SCSS($file, $minify)
	{
		...
		else
		{
			...
			{
				$checklist = Zittme\Framework\Storage::readPHPData($importedFilePath);
		...
		if ($recompile)
		{
			...
			Zittme\Framework\Formatter::$method_name($file->fileFullPath, $compiledFilePath, $file->vars, $minify, $make_sourcemap);
		}
		...
		if ($concat && $finalize && count($concat_list = $this->_concatMap($map)))
		{
			foreach ($concat_list as $concat_fileset)
			{
				...
				else
				{
					...
					{
						$concat_content = Zittme\Framework\Formatter::concatCSS($concat_files, \RX_BASEDIR . $concat_filename);
						$concat_content = '@charset "UTF-8";' . "\n\n" . preg_replace('/@charset\s*[\'"][a-z0-9-]+[\'"];\s*/i', '', $concat_content);
						Zittme\Framework\Storage::write(\RX_BASEDIR . $concat_filename, $concat_content);
					}
		...
		if ($concat && $finalize && $type === 'head' && count($concat_list = $this->_concatMap($map)))
		{
			foreach ($concat_list as $concat_fileset)
			{
				...
				else
				{
					...
					if (!file_exists(\RX_BASEDIR . $concat_filename) || filemtime(\RX_BASEDIR . $concat_filename) < $concat_max_timestamp)
					{
						$concat_content = Zittme\Framework\Formatter::concatJS($concat_files);
						Zittme\Framework\Storage::write(\RX_BASEDIR . $concat_filename, $concat_content);
					}
					$concat_filename .= '?t=' . filemtime(\RX_BASEDIR . $concat_filename);
	...
	{
		$path = Zittme\Framework\Filters\FilenameFilter::cleanPath($path);
