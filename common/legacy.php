 ...
 * @return Zittme\Framework\Helpers\DBResultHelper
 */
function executeQuery($query_id, $args = [], $column_list = [], string $result_type = 'auto', string $result_class = 'stdClass'): Zittme\Framework\Helpers\DBResultHelper
{
 ...
 * @return Zittme\Framework\Helpers\DBResultHelper
 */
function executeQueryArray($query_id, $args = [], $column_list = [], string $result_class = 'stdClass'): Zittme\Framework\Helpers\DBResultHelper
{
	$oDB = Zittme\Framework\DB::getInstance();
	$column_list = is_array($column_list) ? $column_list : [];
...
{
	$seq = Zittme\Framework\DB::getInstance()->getNextSequence();
	setUserSequence($seq);
	return $seq;
}
...
{
	$url = Zittme\Framework\URL::getCurrentURL();
...
{
	return Zittme\Framework\DateTime::getTimezoneOffsetByLegacyFormat((string)$timezone);
}

/**
 * Get the offset between the current user's time zone and Zittme's internal time zone.
 *
...
{
	$current_user_timezone = Zittme\Framework\DateTime::getTimezoneForCurrentUser();
	return Zittme\Framework\DateTime::getTimezoneOffsetFromInternal($current_user_timezone, $timestamp);
}
...
function ztime($str): ?int
{
	...
	if (!$has_offset)
	{
		$offset = $has_time ? (Zittme\Framework\Config::get('locale.internal_timezone') ?: date('Z', $timestamp)) : 0;
	}
...
function zdate($str, $format = 'Y-m-d H:i:s', $conversion = false): ?string
{
	...
	$result = Zittme\Framework\DateTime::formatTimestampForCurrentUser((string)$format, ztime($str));
...
function getInternalDateTime(?int $timestamp = null, string $format = 'YmdHis'): string
{
	...
	return Zittme\Framework\DateTime::formatTimestamp($format, $timestamp);
}
...
{
	$timestamp = ($timestamp !== null) ? $timestamp : time();
	return Zittme\Framework\DateTime::formatTimestampForCurrentUser($format, $timestamp);
}
...
function getTimeGap($date, $format = 'Y.m.d'): ?string
{
	...
	if ($gap < 60 * 60 * 24)
	{
		return Zittme\Framework\DateTime::getRelativeTimestamp(($gap >= 60) ? $timestamp : (RX_TIME - 60));
	}
	else
	{
...
{
	Zittme\Framework\Debug::addEntry($value);

	foreach ($values as $v)
	{
		Zittme\Framework\Debug::addEntry($v);
	}
}
...
{
	return Zittme\Framework\Filters\HTMLFilter::clean((string)$content);
}
...
{
	return Zittme\Framework\UA::isRobot($user_agent);
}
...
function stripEmbedTagForAdmin(&$content, $writer_member_srl): void
{
	...
	if ($logged_info && isset($logged_info->is_admin) && $logged_info->is_admin === 'Y')
	{
		...
		$content = Zittme\Framework\Filters\MediaFilter::removeEmbeddedMedia((string)$content, $security_msg);
	}
 ...
 *
 * Use Zittme\Framework\Security::checkCSRF() instead.
 *
 * @deprecated
 * @return bool
 */
...
{
	return Zittme\Framework\Security::checkCSRF();
}
...
{
	Zittme\Framework\Debug::addError($errno, $errstr, $file, $line, $context);
}
