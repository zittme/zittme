 ...
 */
class Ncloud_Sens extends Base implements \Zittme\Framework\Drivers\SMSInterface
{
	/**
	 * API endpoint URL
	 */
	...
	protected function _uploadFile(string $filename, string $path): ?string
	{
		...
		try
		{
			...
			$request = \Zittme\Framework\HTTP::post($url, [], $headers, [], [
	...
	public function send(array $messages, \Zittme\Framework\SMS $original)
	{
		...
		foreach ($messages as $i => $message)
		{
			...
			{
				$data['messages'][] = [
					'to' => str_replace('-', '', \Zittme\Framework\Korea::formatPhoneNumber($num)),
				];
			}
