...
class Cache
{
	...
	public static function init($config): Drivers\CacheInterface
	{
		...
		if (isset($config['type']))
		{
			...
			$class_name = '\\Zittme\\Framework\\Drivers\\Cache\\' . $config['type'];
			if (isset($config['ttl']))
			{
		...
		elseif (preg_match('/^(apc|dummy|memcached?|redis|sqlite)/', strval(array_first($config)), $matches))
		{
			...
			$class_name = '\\Zittme\\Framework\\Drivers\\Cache\\' . $driver_name;
		}
		else
		{
			$driver_name = null;
	...
	public static function getDriverInstance($name = null, array $config = []): ?Drivers\CacheInterface
	{
		...
		{
			$class_name = '\\Zittme\\Framework\\Drivers\\Cache\\' . $name;
