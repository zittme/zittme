...
namespace Zittme\Framework\Parsers\Template;

use Zittme\Framework\Template;
 ...
 * which are especially important given the modular structure of Zittme.
 *
...
class TemplateParser_v2
{
	 ...
	 *
	 * XE-style syntax:
	 * <use class="Zittme\Modules\Foobar\Models\HelloWorld" as="HelloWorldModel" />
	 *
	 * Blade-style syntax:
	 * @use('Zittme\Modules\Foobar\Models\HelloWorld', 'HelloWorldModel')
	 *
	 * @param string $content
	 * @return string
	 */
	...
	protected function _convertIncludes(string $content): string
	{
		...
		$content = preg_replace_callback($regexp, function($match) {
			...
			$tpl = '<?php $__tpl = new \Zittme\Framework\Template(' . $dir . ', "' . $path . '", "' . ($this->template->extension ?: 'auto') . '"); ';
	 ...
	 * and stylesheets to lang files to Zittme core Javascript plugins.
	 *
	 * XE-style syntax:
	 ...
	 * templates, Zittme will automatically find out which loop you are
	...
	protected function _convertMiscDirectives(string $content): string
	{
		...
		$content = preg_replace_callback('#(?<!@)@csrf\b#', function($match) {
			return '<input type="hidden" name="_rx_csrf_token" value="<?php echo \Zittme\Framework\Session::getGenericToken(); ?>" />';
		}, $content);
