...
class Template
{
	...
	public function parse(?string $content = null): string
	{
		...
		$class_name = '\Zittme\Framework\Parsers\Template\TemplateParser_v' . $this->config->version;
	...
	public function convertPath(string $path, ?string $basepath = null): string
	{
		...
		// Path relative to the Zittme installation directory?
	...
	protected function _v2_include(...$args): string
	{
		...
		// Handle paths relative to the Zittme installation directory.
		if (preg_match('#^\^/?(\w.+)$#s', $path, $match))
		{
	...
	{
		$grant = \Context::get('grant');
		if (!($grant instanceof \Zittme\Modules\Module\Models\Permission))
		{
			return false;
		}
		elseif ($check_type === 1)
