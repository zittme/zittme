...
class Session
{
	 ...
	 * Start the session.
	 *
	 * This method is called automatically at Zittme startup.
	...
	public static function start(bool $force = false): bool
	{
		...
		// Check if the session has been initialized for Zittme.
		if (!isset($_SESSION['Zittme']))
		{
			$must_create = true;
		}

		// Check if the session needs to be refreshed.
		if (!$must_create && $refresh_interval > 0 && (!isset($_SESSION['Zittme']['last_refresh']) || $_SESSION['Zittme']['last_refresh'] < time() - $refresh_interval))
		{
			$must_refresh = true;
		}
		if (!$must_create && isset($_SESSION['Zittme']['next_refresh']) && $_SESSION['Zittme']['next_refresh'] === true)
		{
			$must_refresh = true;
		}

		// If a member is logged in, check if the current session is valid for the member_srl.
		if (isset($_SESSION['Zittme']['login']) && $_SESSION['Zittme']['login'] && !self::isValid($_SESSION['Zittme']['login']))
		{
			trigger_error('Session failed validation checks for member_srl=' . intval($_SESSION['Zittme']['login']), \E_USER_WARNING);
			$_SESSION['Zittme']['login'] = $_SESSION['member_srl'] = false;
			$must_create = true;
		}
		...
		{
			$_SESSION['Zittme']['next_refresh'] = true;
			$must_refresh = false;
		}
		...
		if (isset($_SESSION['Zittme']['autologin_key']) && strlen($_SESSION['Zittme']['autologin_key']) === 48)
		{
			if ($_SESSION['Zittme']['autologin_key'] !== self::_getAutologinKey())
			{
				self::setAutologinKeys(substr($_SESSION['Zittme']['autologin_key'], 0, 24), substr($_SESSION['Zittme']['autologin_key'], 24, 24));
			}
			else
			{
				$_SESSION['Zittme']['autologin_key'] = false;
			}
		}
	 ...
	 *
	 * This method is called automatically at Zittme shutdown.
	...
	{
		// Create the data structure for a new Zittme session.
		$_SESSION['Zittme'] = array();
		$_SESSION['Zittme']['login'] = false;
		$_SESSION['Zittme']['last_login'] = false;
		$_SESSION['Zittme']['last_refresh'] = time();
		$_SESSION['Zittme']['next_refresh'] = false;
		$_SESSION['Zittme']['autologin_key'] = false;
		$_SESSION['Zittme']['ipaddress'] = $_SESSION['ipaddress'] = \RX_CLIENT_IP;
		$_SESSION['Zittme']['useragent'] = isset($_SERVER['HTTP_USER_AGENT']) ? $_SERVER['HTTP_USER_AGENT'] : '';
		$_SESSION['Zittme']['language'] = \Context::getLangType();
		// $_SESSION['Zittme']['timezone'] = DateTime::getTimezoneForCurrentUser();
		$_SESSION['Zittme']['secret'] = Security::getRandom(32, 'alnum');
		$_SESSION['Zittme']['domains'] = array();
		$_SESSION['Zittme']['tokens'] = array();
		$_SESSION['Zittme']['token'] = false;
	...
	public static function refresh(bool $refresh_cookie = false): bool
	{
		...
		if (!isset($_SESSION['Zittme']['domains'][$alt_domain]['started']))
		{
			$_SESSION['Zittme']['domains'][$alt_domain]['started'] = time();
		}
		if (!isset($_SESSION['Zittme']['domains'][$alt_domain]['trusted']))
		{
			$_SESSION['Zittme']['domains'][$alt_domain]['trusted'] = 0;
		}
	...
	public static function close(): void
	{
		...
		if (isset($_SESSION['Zittme']) && $_SESSION['Zittme'] && $_SESSION['Zittme']['login'] !== intval($_SESSION['member_srl']))
		{
			$_SESSION['Zittme']['login'] = intval($_SESSION['member_srl'] ?? 0);
			$_SESSION['Zittme']['last_login'] = time();
	...
	public static function login(int $member_srl, bool $refresh = true): bool
	{
		...
		$_SESSION['Zittme']['login'] = $member_srl;
		$_SESSION['Zittme']['last_login'] = time();
		...
		{
			self::checkLoginStatusCookie();
			return $_SESSION['Zittme']['next_refresh'] = true;
		}
		else
		{
			return true;
	...
	{
		return ($_SESSION['member_srl'] > 0 && $_SESSION['Zittme']['login'] > 0);
	}
	...
	public static function isTrusted(): bool
	{
		...
		if ($_SESSION['Zittme']['domains'][$domain]['trusted'] > time())
		{
			return true;
		}
		else
	...
	public static function isValid(int $member_srl = 0): bool
	{
		...
		$member_srl = intval($member_srl) ?: (isset($_SESSION['Zittme']['login']) ? $_SESSION['Zittme']['login'] : 0);
		if (!$member_srl)
		{
			return false;
		}
		...
		if ($validity_info->invalid_before && self::isStarted() && $_SESSION['Zittme']['last_login'] && $_SESSION['Zittme']['last_login'] < $validity_info->invalid_before)
		{
			trigger_error('Session is invalid for member_srl=' . intval($_SESSION['Zittme']['login']) . ' (expired timestamp)', \E_USER_WARNING);
			return false;
		}
		...
		{
			trigger_error('Session is invalid for member_srl=' . intval($_SESSION['Zittme']['login']) . ' (denied)', \E_USER_WARNING);
			return false;
		}
		if (!empty($member_info->limit_date) && substr($member_info->limit_date, 0, 8) >= date('Ymd'))
		{
			trigger_error('Session is invalid for member_srl=' . intval($_SESSION['Zittme']['login']) . ' (limited)', \E_USER_WARNING);
			return false;
		}
	...
	{
		return intval(($_SESSION['member_srl'] ?? 0) ?: (($_SESSION['Zittme']['login'] ?? 0) ?: 0));
	}
	...
	{
		return isset($_SESSION['Zittme']['language']) ? $_SESSION['Zittme']['language'] : \Context::getLangType();
	}
	...
	{
		$_SESSION['Zittme']['language'] = $language;
	}
	...
	{
		$_SESSION['Zittme']['timezone'] = $timezone;
	}
	...
	public static function setTrusted(int $duration = 300): bool
	{
		...
		if (isset($_SESSION['Zittme']['domains'][$domain]) && $_SESSION['Zittme']['login'])
		{
			$_SESSION['Zittme']['domains'][$domain]['trusted'] = time() + $duration;
			return true;
		}
		else
		{
	...
	public static function getGenericToken()
	{
		...
		if (!$_SESSION['Zittme']['token'])
		{
			$_SESSION['Zittme']['token'] = self::createToken('');
		}

		return $_SESSION['Zittme']['token'];
	}
	...
	public static function createToken(string $key = ''): string
	{
		...
		$_SESSION['Zittme']['tokens'][$token] = $key;
		return $token;
	}
	...
	{
		if (isset($_SESSION['Zittme']['tokens'][$token]) && $_SESSION['Zittme']['tokens'][$token] === strval($key))
		{
			return true;
		}
		elseif (!$strict && empty($_SESSION['Zittme']['tokens']) && !self::getMemberSrl())
		{
			return true;
		}
		else
	...
	{
		if (isset($_SESSION['Zittme']['tokens'][$token]))
		{
			unset($_SESSION['Zittme']['tokens'][$token]);
			return true;
		}
		else
		{
	...
	{
		if (isset($_SESSION['Zittme']) && $_SESSION['Zittme']['login'])
		{
			$data = sprintf('%s:%s:%d:%s', $_SERVER['HTTP_HOST'] ?? '', RX_BASEDIR, $_SESSION['Zittme']['login'], config('crypto.session_key'));
	...
	{
		return $_SESSION['Zittme']['last_login'] ?? 0;
	}
	...
	{
		$key = ($_SESSION['Zittme']['secret'] ?? '') . Config::get('crypto.encryption_key');
	...
	public static function setAutologinKeys(string $autologin_key, string $security_key): bool
	{
		...
		{
			$_SESSION['Zittme']['autologin_key'] = $autologin_key . $security_key;
	...
	public static function destroyOtherSessions(int $member_srl): bool
	{
		...
		if (self::isStarted())
		{
			...
			$_SESSION['Zittme']['last_login'] = $validity_info->invalid_before;
		}
		else
		{
			return false;
