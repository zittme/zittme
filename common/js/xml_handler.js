...
(function($){
	...
	window.exec_xml = $.exec_xml = function(module, act, params, callback_success, return_fields, callback_success_arg, fo_obj) {
		...
		if (typeof console == "object" && typeof console.warn == "function") {
			var msg = "DEPRECATED : exec_xml() is deprecated in Zittme. Use exec_json() instead.";
		...
		successHandler = function(data, textStatus, xhr) {
			...
			if (data.error != "0") {
				...
				if ($.isFunction($.exec_xml.onerror)) {
					if (typeof console == "object" && typeof console.warn == "function") {
						var msg = "DEPRECATED : $.exec_xml.onerror() is deprecated in Zittme.";
						if (navigator.userAgent.match(/Firefox/)) {
							console.error(msg);
						} else {
							console.warn(msg);
	...
	window.exec_html = $.fn.exec_html = function() {
		if (typeof console == "object" && typeof console.warn == "function") {
			var msg = "DEPRECATED : exec_html() is obsolete in Zittme.";
			if (navigator.userAgent.match(/Firefox/)) {
				console.error(msg);
			} else {
				console.warn(msg);
...
function send_by_form(url, params) {
	...
	if (typeof console == "object" && typeof console.warn == "function") {
		var msg = "DEPRECATED : send_by_form() is deprecated in Zittme.";
		if (navigator.userAgent.match(/Firefox/)) {
			console.error(msg);
		} else {
			console.warn(msg);
