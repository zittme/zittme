/**
 * Client-side script for manipulating the debug panel on Zittme.
 *
 * @file debug.js
...
$(function() {
	...
	var panel = $("#Zittme_debug_panel");
	var button = $("#Zittme_debug_button").addClass('visible');
	...
	header.append('<h2>Zittme DEBUG</h2>');
	...
	Zittme.addDebugData = function(data, open) {
	...
	if (Zittme.currentDebugData) {
		Zittme.currentDebugData.page_title = 'MAIN PAGE';
		Zittme.addDebugData(Zittme.currentDebugData, true);
	}

	// Add debug data from pending AJAX requests.
	if (Zittme.pendingDebugData) {
		while (Zittme.pendingDebugData.length) {
			Zittme.addDebugData(Zittme.pendingDebugData.shift());
		}
	}
});
