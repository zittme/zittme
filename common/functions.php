...
function config(string $key, $value = null)
{
	if ($value === null)
	{
		return Zittme\Framework\Config::get($key);
	}
	else
	{
		Zittme\Framework\Config::set($key, $value);
	}
}
...
{
	if (!isset($GLOBALS['lang']) || !$GLOBALS['lang'] instanceof Zittme\Framework\Lang)
	{
		$GLOBALS['lang'] = Zittme\Framework\Lang::getInstance(Context::getLangType() ?: config('locale.default_lang') ?: 'ko');
 ...
 *
 * This function is an alias to Zittme\Framework\Filters\FilenameFilter::cleanPath().
 *
 * @param string $path
 * @return string
 */
...
{
	return Zittme\Framework\Filters\FilenameFilter::cleanPath($path);
}
 ...
 *
 * This function is an alias to Zittme\Framework\URL::fromServerPath().
 * It returns false if the path cannot be converted.
 *
 * @param string $path
 * @return string|false
 */
function path2url(string $path)
{
	return Zittme\Framework\URL::fromServerPath($path);
}

/**
 * Convert a URL to a server-side path.
 *
 * This function is an alias to Zittme\Framework\URL::toServerPath().
 * It returns false if the URL cannot be converted.
 *
 * @param string $url
 * @return string|false
 */
function url2path(string $url)
{
	return Zittme\Framework\URL::toServerPath($url);
}
