...
class Member extends ModuleObject
{
	...
	function checkUpdate()
	{
		...
		// Check social login account table
		if(!$oDB->isTableExists('member_sns_account')) return true;

		// Check identity verification table
		if(!$oDB->isTableExists('member_identity')) return true;
	...
	function moduleUpdate()
	{
		...
		// Create the social login account table if missing
		if(!$oDB->isTableExists('member_sns_account'))
		{
			$oDB->createTableByXmlFile($this->module_path . 'schemas/member_sns_account.xml');
		}

		// Create the identity verification table if missing
		if(!$oDB->isTableExists('member_identity'))
		{
			$oDB->createTableByXmlFile($this->module_path . 'schemas/member_identity.xml');
		}
		...
		if ($config->phone_number_default_country && !preg_match('/^[A-Z]{3}$/', $config->phone_number_default_country))
		{
			$config->phone_number_default_country = Zittme\Framework\i18n::getCountryCodeByCallingCode($config->phone_number_default_country);
			$changed = true;
		}
		$output = executeQuery('member.getMemberCountByPhoneCountry', (object)['phone_country' => '82']);
		if ($output->data->count)
	...
	public function createMid($mid = 'member', $skin = 'default', $mskin = 'default')
	{
		...
		return ModuleController::getInstance()->insertModule((object)array(
			...
			'skin' => $skin,
			'mskin' => $mskin,
			// 회원 모듈은 모바일 대응을 켠 채로 생성한다. 사이트가 반응형이면
			// 그에 해당하는 값이 'R' 이므로, 'Y' 로 굳히면 반응형에서 빠져 버린다.
			'use_mobile' => \Mobile::isResponsiveViewEnabled() ? \Mobile::VIEW_RESPONSIVE : \Mobile::VIEW_MOBILE,
			'isMenuCreate' => false,
		));
	}
