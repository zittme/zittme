...
class MemberAdminController extends Member
{
	...
	function procMemberAdminInsert()
	{
		...
		if($logged_info->is_admin != 'Y' || !Zittme\Framework\Security::checkCSRF())
		{
		...
		{
			$validity_info = Zittme\Framework\Session::getValidityInfo($args->member_srl);
			$validity_info->invalid_before = time();
			Zittme\Framework\Session::setValidityInfo($args->member_srl, $validity_info);
	...
	public function procMemberAdminInsertDefaultConfig()
	{
		...
		if(!array_key_exists($args->password_hashing_algorithm, Zittme\Framework\Password::getSupportedAlgorithms()))
		{
	...
	public function procMemberAdminInsertAgreementsConfig()
	{
		...
		foreach (Context::loadLangSupported() as $key => $val)
		{
			...
			if (Zittme\Framework\Storage::exists($agreement_file))
			{
				Zittme\Framework\Storage::delete($agreement_file);
			}
		}
	...
	public function procMemberAdminInsertSignupConfig()
	{
		...
		if (!array_key_exists($args->phone_number_default_country, Zittme\Framework\i18n::listCountries()))
		{
	...
	public function procMemberAdminInsertSnsLoginConfig()
	{
		$oModuleController = getController('module');

		$args = new stdClass();
		$args->sns_login_enable = Context::get('sns_login_enable') === 'Y' ? 'Y' : 'N';

		foreach(\Zittme\Modules\Member\Social\Base::$supported_services as $service)
		{
			$enable_key = 'sns_' . $service . '_enable';
			$args->{$enable_key} = Context::get($enable_key) === 'Y' ? 'Y' : 'N';
			$args->{$service . '_client_id'} = trim((string)Context::get($service . '_client_id'));
			$args->{$service . '_client_secret'} = trim((string)Context::get($service . '_client_secret'));
		}

		$oModuleController->updateModuleConfig('member', $args);

		$this->setMessage('success_updated');
		$returnUrl = Context::get('success_return_url') ? Context::get('success_return_url') : getNotEncodedUrl('', 'module', 'admin', 'act', 'dispMemberAdminSnsLoginConfig');
		$this->setRedirectUrl($returnUrl);
	}

	public function procMemberAdminInsertIdentityConfig()
	{
		$oModuleController = getController('module');

		$args = new stdClass();
		$args->identity_enable = Context::get('identity_enable') === 'Y' ? 'Y' : 'N';
		$args->identity_dup_check = Context::get('identity_dup_check') === 'Y' ? 'Y' : 'N';
		$args->identity_signup_required = Context::get('identity_signup_required') === 'Y' ? 'Y' : 'N';
		$args->identity_find_account = Context::get('identity_find_account') === 'Y' ? 'Y' : 'N';

		// adult gate targets (mids) + age threshold
		$adult_mids = Context::get('identity_adult_mids');
		$args->identity_adult_mids = is_array($adult_mids) ? array_values(array_filter(array_map('strval', $adult_mids))) : array();
		$adult_age = (int)Context::get('identity_adult_age');
		$args->identity_adult_age = ($adult_age >= 1 && $adult_age <= 99) ? $adult_age : 19;

		// 나이 기준 그룹 편입 규칙: 체크한 그룹만 [group_srl => 최소 나이] 로 저장한다
		$age_group_on = Context::get('identity_age_group_on');
		$age_group_age = Context::get('identity_age_group_age');
		$age_groups = array();
		if (is_array($age_group_on))
		{
			foreach ($age_group_on as $group_srl)
			{
				$group_srl = (int)$group_srl;
				$age = isset($age_group_age[$group_srl]) ? (int)$age_group_age[$group_srl] : 0;
				if ($group_srl > 0 && $age >= 1 && $age <= 99)
				{
					$age_groups[$group_srl] = $age;
				}
			}
		}
		$args->identity_age_groups = $age_groups;

		$provider = strtolower(trim((string)Context::get('identity_provider')));
		$args->identity_provider = in_array($provider, \Zittme\Modules\Member\Identity\Base::$supported_providers, true) ? $provider : 'danal';

		$args->identity_danal_cpid = trim((string)Context::get('identity_danal_cpid'));
		$args->identity_danal_cppwd = trim((string)Context::get('identity_danal_cppwd'));
		$charset = strtoupper(trim((string)Context::get('identity_danal_charset')));
		$args->identity_danal_charset = in_array($charset, ['UTF-8', 'EUC-KR'], true) ? $charset : 'UTF-8';

		// 본인인증을 켜면 인증으로 채워지는 회원 필드(이름·생년월일·휴대폰)를
		// 가입 항목에서 자동 사용 + 비공개로 설정한다.
		if ($args->identity_enable === 'Y')
		{
			$config = getModel('module')->getModuleConfig('member');
			if ($config && is_array($config->signupForm ?? null))
			{
				$identity_fields = array('user_name', 'birthday', 'phone_number');
				$changed = false;
				foreach ($config->signupForm as $signupItem)
				{
					if (in_array($signupItem->name, $identity_fields, true))
					{
						if (empty($signupItem->isUse) || $signupItem->isPublic !== 'N')
						{
							$signupItem->isUse = true;
		...
		$this->setMessage('success_updated');
		$returnUrl = Context::get('success_return_url') ? Context::get('success_return_url') : getNotEncodedUrl('', 'module', 'admin', 'act', 'dispMemberAdminIdentityConfig');
		$this->setRedirectUrl($returnUrl);
	}
	...
	function procMemberAdminInsertJoinForm()
	{
		...
		{
			throw new Zittme\Framework\Exception('msg_column_id_not_available');
		}
		$oMemberModel = getModel('member');
		$config = $oMemberModel->getMemberConfig();
	...
	function procMemberAdminDeleteMembers()
	{
		...
		if(!$target_member_srls) throw new Zittme\Framework\Exceptions\InvalidRequest;
	...
	{
		$member_srl = Context::get('member_srl');
		if(!$member_srl) throw new Zittme\Framework\Exceptions\InvalidRequest;
	...
	function deleteGroup($group_srl)
	{
		...
		if(!$group_info) throw new Zittme\Framework\Exceptions\TargetNotFound;
		if($group_info->is_default == 'Y') throw new Zittme\Framework\Exception('msg_not_delete_default');
	...
	*/
	...
	{
		//remove from cache
		Zittme\Framework\Cache::clearGroup('member');
	}
