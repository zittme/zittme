...
class MemberModel extends Member
{
	...
	public static function getMemberConfig()
	{
		...
		$config->password_hashing_algorithm = $config->password_hashing_algorithm ?? Zittme\Framework\Password::getBestSupportedAlgorithm();
		...
		// Set design config
		// Skin defaults follow the site default design, so that applying a
		// theme (which sets the default member skin) takes effect here too.
		$config->layout_srl = $config->layout_srl ?? 0;
		$config->skin = $config->skin ?? '/USE_DEFAULT/';
		$config->colorset = $config->colorset ?? 'white';
		$config->mlayout_srl = $config->mlayout_srl ?? 0;
		$config->mskin = $config->mskin ?? '/USE_DEFAULT/';
	...
	public function getLoginStatus()
	{
		...
		if ($origin !== '' && $origin !== 'null' && !Zittme\Framework\URL::isInternalURL($origin))
		{
			$this->setError(-1);
		...
		header('Access-Control-Allow-Origin: ' . rtrim(Zittme\Framework\Url::getCurrentDomainURL(), '/'));
		header('Cross-Origin-Resource-Policy: same-origin');

		// Return login status and CSRF token
		Context::setResponseMethod('JSON');
		$this->add('status', Zittme\Framework\Session::getLoginStatus());
		$this->add('csrf_token', Zittme\Framework\Session::getGenericToken());
		$this->add('last_login', Zittme\Framework\Session::getLastLoginTime());
	}
	...
	{
		return Zittme\Framework\Session::getMemberSrl() ? true : false;
	}
	...
	public static function getMemberInfoByPhoneNumber($phone_number, $phone_country = null)
	{
		...
		if($phone_country)
		{
			...
			{
				$phone_country = array($phone_country, preg_replace('/[^0-9]/', '', Zittme\Framework\i18n::getCallingCodeByCountryCode($phone_country)));
			}
			else
			{
				$phone_country = array(preg_replace('/[^0-9]/', '', $phone_country), Zittme\Framework\i18n::getCountryCodeByCallingCode($phone_country));
			}
		}
	...
	public static function getMemberInfoByMemberSrl($member_srl)
	{
		...
		if(!isset($GLOBALS['__member_info__'][$member_srl]))
		{
			...
			$GLOBALS['__member_info__'][$member_srl] = Zittme\Framework\Cache::get($cache_key);
			if(!$GLOBALS['__member_info__'][$member_srl])
			{
				...
				{
					$member_info = self::arrangeMemberInfo($output->data);
					Zittme\Framework\Cache::set($cache_key, $member_info);
				}
				else
				{
	...
	{
		return Zittme\Framework\Session::getMemberSrl();
	}
	...
	public static function getMemberGroups($member_srl, $site_srl = 0, $force_reload = false)
	{
		...
		$group_list = Zittme\Framework\Cache::get($cache_key);
		...
		if(!isset($GLOBALS['__member_groups__'][$member_srl]) || $force_reload)
		{
			if(!$group_list)
			{
				...
				if ($output->toBool())
				{
					Zittme\Framework\Cache::set($cache_key, $group_list, 0, true);
				}
			}
	...
	public static function getDefaultGroup()
	{
		...
		$default_group = Zittme\Framework\Cache::get($cache_key);
		...
		if(!$default_group)
		{
			...
			if($output->toBool())
			{
				Zittme\Framework\Cache::set($cache_key, $default_group, 0, true);
			}
		}
	...
	public static function getGroups()
	{
		...
		{
			$result = array();
			$group_list = Zittme\Framework\Cache::get('member:member_groups');
			...
			if(!$group_list)
			{
				...
				if($output->toBool())
				{
					Zittme\Framework\Cache::set('member:member_groups', $group_list, 0, true);
				}
			}
		...
		foreach($extend_form_list as $srl => $item)
		{
			...
			switch($item->column_type)
			{
				...
				case 'country' :
					...
					$country_list = Zittme\Framework\i18n::listCountries($lang_type === 'ko' ? Zittme\Framework\i18n::SORT_NAME_KOREAN : Zittme\Framework\i18n::SORT_NAME_ENGLISH);
					$country_name = $lang_type === 'ko' ? $country_list[$value]->name_korean : $country_list[$value]->name_english;
					$value = $country_name;
					break;
				case 'language' :
					$supported_lang = Zittme\Framework\Lang::getSupportedList();
	...
	public static function isValidPassword($hashed_password, $password_text, $member_srl=null)
	{
		...
		$possible_algorithms = Zittme\Framework\Password::checkAlgorithm($hashed_password);
		foreach ($possible_algorithms as $algorithm)
		{
			if (Zittme\Framework\Password::checkPassword($password_text, $hashed_password, $algorithm))
			{
				$password_match = true;
		...
		{
			$required_algorithm = Zittme\Framework\Password::getDefaultAlgorithm();
			...
			{
				$required_work_factor = Zittme\Framework\Password::getWorkFactor();
				$current_work_factor = Zittme\Framework\Password::checkWorkFactor($hashed_password);
	...
	{
		return Zittme\Framework\Password::hashPassword($password_text, $algorithm);
	}
