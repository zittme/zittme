...
class MemberController extends Member
{
	...
	function procMemberLogin($user_id = null, $password = null, $keep_signed = null)
	{
		if(!$user_id && !$password && Context::getRequestMethod() == 'GET')
		{
			...
			throw new Zittme\Framework\Exception('null_user_id');
		}
		...
		if (!$password)
		{
			throw new Zittme\Framework\Exception('null_password');
		}
		...
		Zittme\Modules\Member\Controllers\Device::getInstance()->autoRegisterDevice($member_info->member_srl);
	...
	function procMemberLogout()
	{
		...
		Zittme\Framework\Session::logout();
		self::clearMemberCache($logged_info->member_srl);

		// Discard identity verification results when the session owner changes (shared PC protection)
		\Zittme\Modules\Member\Identity\Base::clearVerified();
		...
		Zittme\Modules\Member\Controllers\Device::getInstance()->autoUnregisterDevice($logged_info->member_srl);
		if (isset($_COOKIE['device_key']))
		{
			Zittme\Framework\Cookie::remove('device_key');
		}

		// Set redirect URL.
		$output = new BaseObject();
		$redirect_url = Context::get('redirect_url');
		if ($redirect_url && Zittme\Framework\URL::isInternalURL($redirect_url))
		{
	...
	function procMemberScrapDocument()
	{
		...
		{
			throw new Zittme\Framework\Exception('msg_is_secret');
		}
		...
		{
			throw new Zittme\Framework\Exception('msg_alreay_scrapped');
		}
	...
	function procMemberDeleteScrap()
	{
		...
		if(!Context::get('is_logged')) throw new Zittme\Framework\Exceptions\MustLogin;
		$logged_info = Context::get('logged_info');
	...
	function procMemberInsertScrapFolder()
	{
		...
		{
			throw new Zittme\Framework\Exception('msg_folder_alreay_exists');
		}
	...
	function procMemberRenameScrapFolder()
	{
		...
		{
			throw new Zittme\Framework\Exception('msg_folder_is_default');
		}
	...
	function procMemberDeleteScrapFolder()
	{
		...
		if(count($output->data))
		{
			throw new Zittme\Framework\Exception('msg_folder_not_empty');
		}
	...
	function procMemberDeleteSavedDocument()
	{
		...
		$document_srl = intval(Context::get('document_srl') ?: Context::get('target_srl'));
		if(!$document_srl) throw new Zittme\Framework\Exceptions\InvalidRequest;
	...
	function procMemberInsert()
	{
		...
		// Identity-required signup: a valid verification result must exist in the session.
		// Results are only stored via server-to-server CONFIRM, so they cannot be forged.
		$identity_verified = null;
		if (\Zittme\Modules\Member\Identity\Base::isEnabled())
		{
			// Always record the verification result for the new member, regardless of the
			// "required" setting; otherwise the adult gate reappears for verified members.
			$identity_verified = \Zittme\Modules\Member\Identity\Base::getVerified();
			if (!$identity_verified
				&& \Zittme\Modules\Member\Identity\Base::getConfig()->identity_signup_required === 'Y')
			{
				throw new Zittme\Framework\Exception('msg_identity_required');
			}
			// duplicate-signup guard at signup time (CI = 1 person 1 account)
			if ($identity_verified
				&& \Zittme\Modules\Member\Identity\Base::getConfig()->identity_dup_check === 'Y'
				&& \Zittme\Modules\Member\Identity\IdentityModel::getInstance()->getByCi((string)$identity_verified['ci']))
			{
				throw new Zittme\Framework\Exception('identity_ci_duplicate');
			}
		}
		...
		foreach($config->agreements as $i => $agreement)
		{
			...
			{
				throw new Zittme\Framework\Exception('msg_accept_agreement');
			}
		...
		// Identity-required signup: overwrite name/birthday/phone with verified values.
		// Form input is ignored to prevent forgery.
		if ($identity_verified)
		{
			if (!empty($identity_verified['name']))
			{
				$args->user_name = $identity_verified['name'];
			}
			if (!empty($identity_verified['birthday']))
			{
				$args->birthday = intval($identity_verified['birthday']);
			}
			if ($use_phone && !empty($identity_verified['phone']))
			{
				$args->phone_number = $identity_verified['phone'];
				$args->phone_country = 'KOR';
			}
		}
		...
		if(!MemberModel::checkPasswordStrength($args->password, $config->password_strength))
		{
			...
			throw new Zittme\Framework\Exception($message[$config->password_strength]);
		}
		...
		if($config->nickname_symbols === 'N')
		{
			...
			{
				throw new Zittme\Framework\Exception(sprintf(lang('msg_invalid_symbol_in_nickname'), escape($matches[0])));
			}
		}
		...
		// Identity-required signup: record the verified result and consume the session
		if ($identity_verified)
		{
			\Zittme\Modules\Member\Identity\IdentityModel::getInstance()->saveForMember((int)$args->member_srl, $identity_verified);
			// Age-based group assignment (only if rules are enabled)
			\Zittme\Modules\Member\Identity\Base::applyAgeGroups((int)$args->member_srl, (string)($identity_verified['birthday'] ?? ''));
			\Zittme\Modules\Member\Identity\Base::clearVerified();
		}
		...
		// Register device
		Zittme\Modules\Member\Controllers\Device::getInstance()->autoRegisterDevice($args->member_srl, false);
	...
	function procMemberModifyInfoBefore()
	{
		...
		// Social-only members have no password set (empty hash). Their identity is
		// already established by the logged-in session, so skip the password re-check.
		...
		else
		{
			...
			{
				throw new Zittme\Framework\Exception('invalid_password');
			}
	...
	function procMemberModifyInfo()
	{
		...
		// Verified fields cannot be edited by the member; keep stored values.
		// They can only be updated by verifying again.
		$identity_record = \Zittme\Modules\Member\Identity\IdentityModel::getInstance()->getByMemberSrl((int)$logged_info->member_srl);
		if ($identity_record)
		{
			if (!empty($identity_record->name))
			{
				$args->user_name = $identity_record->name;
			}
			if (!empty($identity_record->birthday))
			{
				$args->birthday = intval($identity_record->birthday);
			}
			if ($use_phone && !empty($identity_record->phone))
			{
				$args->phone_number = $identity_record->phone;
				$args->phone_country = 'KOR';
			}
		}
	...
	public function procMemberModifyPassword()
	{
		...
		{
			throw new Zittme\Framework\Exception('invalid_current_password');
		}

		// Check if a new password is as same as the previous password
		if ($current_password === $password)
		{
			throw new Zittme\Framework\Exception('invalid_new_password');
		}
		...
		{
			Zittme\Framework\Session::destroyOtherSessions($member_srl);
		}
	...
	public function procMemberResetPassword()
	{
		...
		if(!$output->toBool() || $output->data->auth_key !== $vars->auth_key)
		{
			...
			throw new Zittme\Framework\Exception('msg_invalid_auth_key');
		}
		...
		{
			executeQuery('member.deleteAuthMail', ['auth_key' => $vars->auth_key]);
			throw new Zittme\Framework\Exception('msg_expired_auth_key');
		}
	...
	function procMemberLeave()
	{
		...
		$member_srl = $logged_info->member_srl;
		$member_info = MemberModel::getMemberInfoByMemberSrl($member_srl);
		// Verify the current password. Social-only members have no password set
		// (empty hash) and are identified by their logged-in session, so skip it.
		if ($member_info->password && !MemberModel::isValidPassword($member_info->password, $password))
		{
			throw new Zittme\Framework\Exception('invalid_password');
		}
	...
	/**
	 * Social login: redirect the visitor to the provider's authorization page.
	 *
	 * @return void|BaseObject
	 */
	function procMemberSnsConnect()
	{
		$service = strtolower((string)Context::get('service'));

		if(!\Zittme\Modules\Member\Social\Base::isServiceEnabled($service))
		{
			throw new Zittme\Framework\Exceptions\InvalidRequest;
		}

		$driver = \Zittme\Modules\Member\Social\Base::getDriver($service);
		if(!$driver)
		{
			throw new Zittme\Framework\Exceptions\InvalidRequest;
		}

		// Remember where to return after login. Prefer an explicit internal
		// redirect param, otherwise the referring page (but never the callback).
		$redirect = (string)Context::get('redirect');
		if($redirect !== '' && \Zittme\Framework\URL::isInternalURL($redirect))
		{
			$_SESSION['member_sns_auth']['referer'] = $redirect;
		}
		else
		{
			$referer = $_SERVER['HTTP_REFERER'] ?? '';
			if($referer && strpos($referer, 'procMemberSnsCallback') === false)
			{
				$_SESSION['member_sns_auth']['referer'] = $referer;
			}
		}

		$authUrl = $driver->createAuthUrl();
	...
	/**
	 * Social login: OAuth callback. Links or creates a member and logs in.
	 *
	 * @return void|BaseObject
	 */
	function procMemberSnsCallback()
	{
		$service = strtolower((string)Context::get('service'));
		...
		// The user cancelled/denied on the consent screen (no authorization code):
		// return silently to the originating page instead of showing an error.
		if(!Context::get('code'))
		{
		...
		$authResult = $driver->authenticate();
		if($authResult instanceof BaseObject && !$authResult->toBool())
		{
			return $authResult;
		}
		$access_token = $authResult['access_token'];

		// 2. Fetch the provider-side profile.
		$userInfo = $driver->getUserInfo($access_token);
		if(!$userInfo)
		{
			return new BaseObject(-1, 'msg_errer_api_connect');
		}

		$oAccountModel = \Zittme\Modules\Member\Social\AccountModel::getInstance();
		$logged_info = Context::get('logged_info');

		// 3. Existing linked account?
		$account = $oAccountModel->getAccount($userInfo->sns_type, $userInfo->sns_id);

		// The linked member may have been deleted; drop the stale link and fall through.
		if($account)
		{
			$member_exists = false;
			if($account->member_srl)
			{
				$memberInfo = MemberModel::getMemberInfoByMemberSrl($account->member_srl);
				if($memberInfo && $memberInfo->member_srl)
				{
					$member_exists = true;
				}
			}
			if(!$member_exists)
			{
				$oAccountModel->deleteAccount($account->member_srl, $account->sns_type);
				$account = null;
			}
		}

		// [Case A] Already logged in — link the social account to this member.
		if($logged_info)
		{
			if($account)
			{
				if($account->member_srl == $logged_info->member_srl)
				{
					$redirectUrl = getNotEncodedFullUrl('', 'act', 'dispMemberSnsAccounts');
					header('Location: ' . $redirectUrl);
					exit;
				}
				return $this->_snsAlertAndBack('msg_sns_already_linked');
			}

			$oAccountModel->insertAccount([
				'member_srl' => $logged_info->member_srl,
				'sns_type' => $userInfo->sns_type,
				'sns_id' => $userInfo->sns_id,
				'sns_name' => $userInfo->sns_name,
				'sns_email' => $userInfo->sns_email,
				'sns_profile_image' => $userInfo->sns_profile_image,
				'access_token' => $access_token,
				'regdate' => date('YmdHis'),
				'last_login' => date('YmdHis'),
			]);

			if($userInfo->sns_profile_image && empty($logged_info->profile_image))
			{
				$this->_snsDownloadProfileImage($logged_info->member_srl, $userInfo->sns_profile_image);
			}
		...
		// No linked account — auto-link a verified-email member, else sign up.
		if($userInfo->sns_email && strpos($userInfo->sns_email, '@sns.local') === false)
		{
			$existMember = MemberModel::getMemberInfoByEmailAddress($userInfo->sns_email);
			if($existMember && $existMember->member_srl)
			{
				$canAutoLink = ($userInfo->sns_type === 'google') || !empty($userInfo->sns_email_verified);
				if(!$canAutoLink)
				{
					return $this->_snsAlertAndBack('msg_email_already_exists');
				}

				$oAccountModel->insertAccount([
					'member_srl' => $existMember->member_srl,
					'sns_type' => $userInfo->sns_type,
					'sns_id' => $userInfo->sns_id,
					'sns_name' => $userInfo->sns_name,
					'sns_email' => $userInfo->sns_email,
		...
		// New signup — create a password-less member (social-only account).
		$member_srl = $this->_snsCreateMember($userInfo, $access_token);
		if($member_srl instanceof BaseObject)
		{
			return $member_srl;
		}
	...
	/**
	 * Social login: unlink a provider from the logged-in member.
	 *
	 * @return void|BaseObject
	 */
	function procMemberSnsUnlink()
	{
		if(!Context::get('is_logged'))
		{
			throw new Zittme\Framework\Exceptions\MustLogin;
		...
		$logged_info = Context::get('logged_info');
		$service = strtolower((string)Context::get('sns_type'));
		if($service === '')
		{
			throw new Zittme\Framework\Exceptions\InvalidRequest;
		}

		\Zittme\Modules\Member\Social\AccountModel::getInstance()->deleteAccount($logged_info->member_srl, $service);

		$this->setMessage('success_deleted');
		$this->setRedirectUrl(getNotEncodedUrl('', 'act', 'dispMemberSnsAccounts'));
	}

	/**
	 * Identity verification callback. The provider POSTs a TID, which is then
	 * verified via server-to-server confirm; values passed through the browser
	 * are not trusted. Runs in a popup: posts the result to the opener and closes.
	 */
	function procMemberIdentityCallback()
	{
		// user cancelled at the provider (BackURL).
		// The provider may strip extra return-URL parameters, so treat a missing TID
		// as cancellation instead of relying on the cancel flag alone.
		if(Context::get('IDENTITY_CANCEL') === 'Y' || (empty($_POST['TID']) && empty($_GET['TID'])))
		{
			$this->_identityPopupFinish(false, lang('identity_cancelled'));
		}

		if(!\Zittme\Modules\Member\Identity\Base::isEnabled())
		{
			$this->_identityPopupFinish(false, lang('msg_invalid_request'));
		}

		// SESSION-FREE: the provider posts this callback CROSS-SITE, so the
		// session cookie may be absent (SameSite) — touching the session here
		// would mint a new one and invalidate the opener's CSRF tokens.
		// Correlation + result hand-off use the file ticket store instead;
		// the opener claims the ticket into its own session afterwards.
		$state = (string)Context::get('IDENTITY_STATE');
		if(!\Zittme\Modules\Member\Identity\Base::isTicketValid($state))
		{
			$this->_identityPopupFinish(false, lang('msg_invalid_request'));
		}

		$driver = \Zittme\Modules\Member\Identity\Base::getDriver();
		$result = $driver ? $driver->confirm($_POST) : ['success' => false, 'message' => 'driver missing'];

		if(empty($result['success']))
		{
			$this->_identityPopupFinish(false, $result['message'] ?? lang('identity_failed'));
		}
		if(empty($result['ci']))
		{
			$this->_identityPopupFinish(false, lang('identity_failed'));
		}

		// store under the ticket; policies (duplicates etc.) run at claim time
		\Zittme\Modules\Member\Identity\Base::storeTicketResult($state, $result);

		$this->_identityPopupFinish(true, lang('identity_success'), [
			'ticket' => $state,
	...
	/**
	 * Adult verification gate. Entry to designated mids is limited to members
	 * who passed identity verification and the age threshold; others are
	 * redirected to the verification page. (moduleHandler.init after trigger)
	 */
	function triggerIdentityAdultGate(&$module_info)
	{
		// gate only mid-based front pages
		if (empty($module_info->mid))
		{
			return;
		}

		$config = \Zittme\Modules\Member\Identity\Base::getConfig();
		if ($config->identity_enable !== 'Y' || empty($config->identity_adult_mids))
		{
			return;
		}
		if (!in_array($module_info->mid, $config->identity_adult_mids, true))
		{
			return;
		}
		...
		// verified adult member passes
		if ($logged_info && !empty($logged_info->member_srl))
		{
			$verified = \Zittme\Modules\Member\Identity\IdentityModel::getInstance()
		...
		// non-HTML (ajax/JSON/POST) requests: plain error
		if (Context::getRequestMethod() !== 'GET' || Context::getResponseMethod() !== 'HTML')
		{
			return new BaseObject(-1, 'msg_identity_adult_required');
		}

		// redirect to the gate page with a same-site return URL
		// getNotEncodedFullUrl('') reassembles the URL from request vars and loses
		// the path on rewritten URLs. Use the raw REQUEST_URI instead, which is
		// guaranteed to be on this site.
		$protocol = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https' : 'http';
		$request_uri = (string)($_SERVER['REQUEST_URI'] ?? '');
		$return_url = ($request_uri !== '' && $request_uri[0] === '/')
			? $protocol . '://' . \Zittme\Framework\URL::getCurrentDomain(true) . $request_uri
			: getNotEncodedFullUrl('');
		$gate_url = \Zittme\Modules\Member\Identity\Drivers\Base::buildActionUrl('dispMemberIdentityGate', ['return_url' => $return_url]);
	...
	/**
	 * Account recovery via identity verification: reset the password of the
	 * account matching the verified CI. Verification itself proves identity,
	 * so no e-mail confirmation is needed.
	 */
	function procMemberIdentityResetPassword()
	{
		$config = \Zittme\Modules\Member\Identity\Base::getConfig();
		if(!\Zittme\Modules\Member\Identity\Base::isEnabled() || $config->identity_find_account !== 'Y')
		{
		...
		if(!$record)
		{
			throw new Zittme\Framework\Exception('identity_no_account');
		}

		$password = (string)Context::get('new_password');
		$password2 = (string)Context::get('new_password2');
		if($password === '' || $password !== $password2)
		{
			throw new Zittme\Framework\Exception('msg_password_mismatch');
		}

		$args = new stdClass;
		$args->member_srl = (int)$record->member_srl;
		$args->password = $password;
		$output = $this->updateMemberPassword($args);
		if(!$output->toBool())
		{
		...
		$this->setMessage('identity_password_changed');
		$this->setRedirectUrl(getNotEncodedUrl('', 'act', 'dispMemberLoginForm'));
	}

	/**
	 * Print the popup-closing page (postMessage to opener) and stop.
	 */
	private function _identityPopupFinish($success, $message, array $data = [])
	{
		$payload = json_encode([
			'type' => 'zittme:identity',
			'success' => (bool)$success,
			'message' => (string)$message,
			'data' => $data,
		], JSON_UNESCAPED_UNICODE);

		// Never send a session cookie here. The provider callback is a cross-site
		// POST without our session cookie, so the framework would issue a new one
		// that overwrites the parent window session and invalidates its CSRF token.
		if (!headers_sent())
		{
		...
		echo '<!DOCTYPE html><html><head><meta charset="utf-8"><title>본인인증</title></head><body>';
		echo '<script>';
		echo 'try { if (window.opener) { window.opener.postMessage(' . $payload . ', window.location.origin); } } catch(e) {}';
		if(!$success)
		{
			echo 'alert(' . json_encode((string)$message, JSON_UNESCAPED_UNICODE) . ');';
		}
		echo 'window.close();';
		echo '</script>';
		echo '<p style="text-align:center;padding:40px 16px;font-size:14px;color:#667085;">' . htmlspecialchars((string)$message, ENT_QUOTES, 'UTF-8') . '</p>';
		echo '</body></html>';
		Context::close();
		exit;
	}
	 ...
	 */
	private function _snsCreateMember($userInfo, $access_token)
	{
		$nick_name = trim($userInfo->sns_name) ?: 'social';
		if(MemberModel::getMemberSrlByNickName($nick_name))
		{
			$i = 1;
			while(MemberModel::getMemberSrlByNickName($nick_name . $i))
			{
				$i++;
			}
		...
		// Provider prefix: kakao->ka, naver->na, google->go.
		$prefix = substr($userInfo->sns_type, 0, 2);
		$has_real_email = ($userInfo->sns_email && strpos($userInfo->sns_email, '@') !== false);

		// Insert first (member_srl is assigned inside insertMember), using a
		// temporary unique id/email, then rewrite both to the "{prefix}_{srl}" form.
		$temp_key = substr(sha1($userInfo->sns_type . '|' . $userInfo->sns_id . '|' . uniqid('', true)), 0, 12);
		$args = new stdClass();
		$args->user_id = 'sns' . $temp_key;
		$args->nick_name = $nick_name;
		$args->email_address = $has_real_email ? $userInfo->sns_email : ($temp_key . '@sns.local');
		$args->password = ''; // social-only member: no password (see the password gate)
		$args->user_name = $userInfo->sns_name ?: $nick_name;
		...
		// Rewrite to the clean, short "{prefix}_{member_srl}" identity.
		$user_id = $prefix . '_' . $member_srl;
		$upd = new stdClass();
		$upd->member_srl = $member_srl;
		$upd->user_id = $user_id;
		if(!$has_real_email)
		{
			$upd->email_address = $user_id . '@sns.local';
			$upd->email_id = $user_id;
			$upd->email_host = 'sns.local';
		}
		executeQuery('member.updateMemberSnsIdentity', $upd);
		self::clearMemberCache($member_srl);

		\Zittme\Modules\Member\Social\AccountModel::getInstance()->insertAccount([
	...
	/**
	 * Download a remote profile image and register it as the member's image.
	 */
	private function _snsDownloadProfileImage($member_srl, $image_url)
	{
		try
		{
			$temp_file = \RX_BASEDIR . 'files/cache/tmp_sns_profile_' . $member_srl . '.jpg';
			if(\FileHandler::getRemoteFile($image_url, $temp_file))
			{
				if(file_exists($temp_file) && filesize($temp_file) > 0)
				{
					$this->insertProfileImage($member_srl, $temp_file);
					@unlink($temp_file);
				}
			}
		}
	...
	/**
	 * Show a JS alert and return to the previous page.
	 */
	...
	function procMemberInsertProfileImage()
	{
		...
		if(!is_uploaded_file($file['tmp_name'])) throw new Zittme\Framework\Exception('msg_not_uploaded_profile_image');
		// Ignore if member_srl is invalid or doesn't exist.
		$member_srl = Context::get('member_srl');
		if(!$member_srl) throw new Zittme\Framework\Exception('msg_not_uploaded_profile_image');

		$logged_info = Context::get('logged_info');
		if($logged_info->is_admin != 'Y' && $logged_info->member_srl != $member_srl) throw new Zittme\Framework\Exception('msg_not_uploaded_profile_image');
		// Return if member module is set not to use an image name or the user is not an administrator ;
		$config = MemberModel::getMemberConfig();
		if($logged_info->is_admin != 'Y' && $config->profile_image != 'Y') throw new Zittme\Framework\Exception('msg_not_uploaded_profile_image');
	...
	function insertProfileImage($member_srl, $target_file)
	{
		...
		{
			throw new Zittme\Framework\Exception('msg_not_uploaded_profile_image');
		}
		...
		if ($resize || $rotate)
		{
			...
			if($max_filesize && $filesize > ($max_filesize * 1024))
			{
				...
				throw new Zittme\Framework\Exception(implode(' ' , array(
	...
	function procMemberInsertImageName()
	{
		...
		if(!is_uploaded_file($file['tmp_name'])) throw new Zittme\Framework\Exception('msg_not_uploaded_image_name');
		// Ignore if member_srl is invalid or doesn't exist.
		$member_srl = Context::get('member_srl');
		if(!$member_srl) throw new Zittme\Framework\Exception('msg_not_uploaded_image_name');

		$logged_info = Context::get('logged_info');
		if($logged_info->is_admin != 'Y' && $logged_info->member_srl != $member_srl) throw new Zittme\Framework\Exception('msg_not_uploaded_image_name');
		// Return if member module is set not to use an image name or the user is not an administrator ;
		$config = MemberModel::getMemberConfig();
		if($logged_info->is_admin != 'Y' && $config->image_name != 'Y') throw new Zittme\Framework\Exception('msg_not_uploaded_image_name');
	...
	function procMemberDeleteProfileImage($_memberSrl = 0)
	{
		...
		if($logged_info && ($logged_info->is_admin == 'Y' || $logged_info->member_srl == $member_srl))
		{
			...
			if(!empty($profile_image->file))
			{
				...
				Zittme\Framework\Storage::deleteEmptyDirectory(dirname(FileHandler::getRealPath($profile_image->file)), true);
	...
	function procMemberDeleteImageName($_memberSrl = 0)
	{
		...
		if($logged_info && ($logged_info->is_admin == 'Y' || $logged_info->member_srl == $member_srl))
		{
			...
			if(!empty($image_name->file))
			{
				...
				Zittme\Framework\Storage::deleteEmptyDirectory(dirname(FileHandler::getRealPath($image_name->file)), true);
				FileHandler::clearStatCache($profile_image->file);
				self::clearMemberCache($member_srl);
			}
		}
	...
	function procMemberInsertImageMark()
	{
		...
		if(!is_uploaded_file($file['tmp_name'])) throw new Zittme\Framework\Exception('msg_not_uploaded_image_mark');
		// Ignore if member_srl is invalid or doesn't exist.
		$member_srl = Context::get('member_srl');
		if(!$member_srl) throw new Zittme\Framework\Exception('msg_not_uploaded_image_mark');

		$logged_info = Context::get('logged_info');
		if($logged_info->is_admin != 'Y' && $logged_info->member_srl != $member_srl) throw new Zittme\Framework\Exception('msg_not_uploaded_image_mark');
		// Membership in the images mark the module using the ban was set by an administrator or return;
		$config = MemberModel::getMemberConfig();
		if($logged_info->is_admin != 'Y' && $config->image_mark != 'Y') throw new Zittme\Framework\Exception('msg_not_uploaded_image_mark');
	...
	function procMemberDeleteImageMark($_memberSrl = 0)
	{
		...
		if($logged_info && ($logged_info->is_admin == 'Y' || $logged_info->member_srl == $member_srl))
		{
			...
			if(!empty($image_mark->file))
			{
				...
				Zittme\Framework\Storage::deleteEmptyDirectory(dirname(FileHandler::getRealPath($image_mark->file)), true);
				FileHandler::clearStatCache($profile_image->file);
				self::clearMemberCache($member_srl);
			}
		}
	...
	function procMemberFindAccount()
	{
		...
		{
			throw new Zittme\Framework\Exception('msg_rate_limited_find_account');
		}

		// Check email address
		$email_address = Context::get('email_address');
		if(!$email_address) throw new Zittme\Framework\Exceptions\InvalidRequest;

		// Check if a member having the same email address exists
		$member_srl = MemberModel::getMemberSrlByEmailAddress($email_address);
		if(!$member_srl) throw new Zittme\Framework\Exception('msg_not_exists_member');

		// Get information of the member
		$member_info = MemberModel::getMemberInfoByMemberSrl($member_srl);
		if(!$member_info || !$member_info->member_srl)
		{
			throw new Zittme\Framework\Exception('msg_not_exists_member');
		}
		...
		if($member_info->denied == 'Y')
		{
			...
			{
				throw new Zittme\Framework\Exception(sprintf('msg_user_not_confirmed', $member_info->email_address));
			}
		}
		...
		$args->member_srl = $member_info->member_srl;
		$args->new_password = $password_reset_method == 2 ? '' : Zittme\Framework\Password::getRandomPassword(8);
		$args->auth_key = Zittme\Framework\Security::getRandom(40, 'hex');
		...
		if (in_array('phone_number', $member_config->identifiers))
		{
			...
			{
				$phone_number = Zittme\Framework\i18n::formatPhoneNumber($phone_number, $member_info->phone_country);
			}
			elseif($member_config->phone_number_default_country === 'KOR' && ($member_info->phone_country === 'KOR' || $member_info->phone_country == '82'))
			{
				$phone_number = Zittme\Framework\Korea::formatPhoneNumber($phone_number);
			}
		...
		$oTemplate = new Zittme\Framework\Template($tpl_path, 'find_member_account_mail');
		$content = $oTemplate->compile();

		// Wrap the skin-generated body in the common mail layout
		$content = \Zittme\Framework\MailTemplate::wrap(lang('msg_find_account_title'), $content, array(
			'greeting' => sprintf(lang('mail_greeting'), $member_info->nick_name),
			'button_url' => $find_url,
			'button_label' => lang('cmd_find_member_account'),
		));
		...
		// Send a mail
		$oMail = new \Zittme\Framework\Mail();
	...
	{
		throw new Zittme\Framework\Exception('msg_question_not_allowed');
	}
	...
	function procMemberAuthAccount()
	{
		...
		else
		{
			...
			if (!Zittme\Framework\Storage::exists($tpl_file))
			{
		...
		// Verification data is cleared at login time, not here
	...
	function procMemberResendAuthMail()
	{
		...
		{
			throw new Zittme\Framework\Exception('msg_activation_not_needed');
		}
		...
		if($output->toBool() && $output->data->count == '0')
		{
			throw new Zittme\Framework\Exception('msg_activation_key_not_found');
		}
		...
		$oTemplate = new Zittme\Framework\Template($tpl_path, 'confirm_member_account_mail');
		$content = $oTemplate->compile();

		// Wrap the skin-generated body in the common mail layout
		$content = \Zittme\Framework\MailTemplate::wrap(lang('msg_confirm_account_title'), $content, array(
			'greeting' => sprintf(lang('mail_greeting'), $member_info->nick_name),
			'button_url' => $auth_url,
			'button_label' => lang('cmd_confirm_account'),
		));
	...
	public static function putSignature($member_srl, $signature)
	{
		...
		Zittme\Framework\Storage::write($filename, $buff);
	...
	public static function delSignature($member_srl)
	{
		...
		Zittme\Framework\Storage::delete($filename);
		Zittme\Framework\Storage::deleteEmptyDirectory($dirname, true);
		clearstatcache(true, $filename);
		self::clearMemberCache($member_srl);
	}
	...
	public static function doAutoLogin($autologin_key = null)
	{
		...
		{
			$new_security_key = Zittme\Framework\Security::getRandom(24, 'alnum');
			...
			$args->user_agent = json_encode(Zittme\Framework\UA::getBrowserInfo());
			$update_output = executeQuery('member.updateAutologin', $args);
			if ($update_output->toBool())
			{
				Zittme\Framework\Session::setAutologinKeys($autologin_key, $new_security_key);
			}
		}
		else
		{
			...
			if ($update_output->toBool())
			{
				Zittme\Framework\Session::setAutologinKeys($autologin_key, $security_key);
			}
		}
	...
	function doLogin($user_id, $password = '', $keep_signed = false)
	{
		...
		elseif($config->identifiers && in_array('phone_number', $config->identifiers) && strpos($user_id, '@') === false && !preg_match('/^[a-z]/i', $user_id))
		{
			...
			{
				$phone_country = Zittme\Framework\i18n::getCountryCodeByCallingCode($phone_country);
			}
		...
		if ($output->data->count >= $config->max_error_count)
		{
			...
			{
				if (!$config->login_failure_except_ip || !Zittme\Framework\Filters\IpFilter::inRanges(\RX_CLIENT_IP, $config->login_failure_except_ip))
				{
		...
		if($keep_signed)
		{
			$random_key = Zittme\Framework\Security::getRandom(48, 'alnum');
			...
			$autologin_args->user_agent = json_encode(Zittme\Framework\UA::getBrowserInfo());
			$autologin_output = executeQuery('member.insertAutologin', $autologin_args);
			if ($autologin_output->toBool())
			{
				Zittme\Framework\Session::setAutologinKeys(substr($random_key, 0, 24), substr($random_key, 24, 24));
			}
		}
	...
	public function setSessionInfo()
	{
		...
		$member_info = Zittme\Framework\Session::getMemberInfo(true);
		...
		{
			$this->addMemberMenu( 'dispMemberSnsAccounts', 'sns_my_services');
		}
		if (\Zittme\Modules\Member\Identity\Base::isEnabled())
		{
			$this->addMemberMenu( 'dispMemberIdentityStatus', 'identity_verification');
		}
	}
	...
	function insertMember(&$args, $password_is_hashed = false)
	{
		...
		if (strval($args->phone_number) !== '')
		{
			...
			{
				$args->phone_country = Zittme\Framework\i18n::getCountryCodeByCallingCode($args->phone_country);
			}
			if ($args->phone_country === 'KOR' && !Zittme\Framework\Korea::isValidPhoneNumber($args->phone_number))
			{
		...
		if($args->denied == 'Y')
		{
			...
			$auth_args->auth_key = Zittme\Framework\Security::getRandom(40, 'hex');
	...
	function deleteMember($member_srl)
	{
		...
		// Delete linked social accounts and identity verification records.
		// (tables may not exist on sites that never ran the module update)
		if($oDB->isTableExists('member_sns_account'))
		{
			executeQuery('member.deleteMemberSnsAccount', $args);
		}
		if($oDB->isTableExists('member_identity'))
		{
			executeQuery('member.deleteMemberIdentity', $args);
		}
		...
		$dirs = Zittme\Framework\Storage::readDirectory(RX_BASEDIR . 'files/member_extra_info', true, true, false);
		foreach ($dirs as $dir)
		{
			$member_dir = $dir . '/' . getNumberingPath($member_srl);
			Zittme\Framework\Storage::deleteDirectory($member_dir, false);
			Zittme\Framework\Storage::deleteEmptyDirectory($member_dir, true);
		}
	...
	{
		Zittme\Framework\Session::destroy();
	}
	...
	function procMemberModifyEmailAddress()
	{
		...
		if(MemberModel::isDeniedEmailHost($newEmail))
		{
			...
			$message = sprintf($managed_email_host[$emailhost_check],implode(', ',$hosts),'id@'.implode(', id@',$hosts));
			throw new Zittme\Framework\Exception($message);
		}

		// Check if the e-mail address is already registered
		$member_srl = MemberModel::getMemberSrlByEmailAddress($newEmail);
		if($member_srl) throw new Zittme\Framework\Exception('msg_exists_email_address');
		...
		$oTemplate = new Zittme\Framework\Template($tpl_path, 'confirm_member_new_email');
		$content = $oTemplate->compile();

		// Wrap the skin-generated body in the common mail layout
		$content = \Zittme\Framework\MailTemplate::wrap(lang('title_modify_email_address'), $content, array(
			'greeting' => sprintf(lang('mail_greeting'), $member_info->nick_name),
			'button_url' => $auth_url,
			'button_label' => lang('cmd_confirm_account'),
		));
	...
	function procMemberAuthEmailAddress()
	{
		$member_srl = Context::get('member_srl');
		$auth_key = Context::get('auth_key');
		if(!$member_srl || !$auth_key) throw new Zittme\Framework\Exceptions\InvalidRequest;
		...
		if(!$output->toBool() || $output->data->auth_key != $auth_key)
		{
			...
			throw new Zittme\Framework\Exception('msg_invalid_modify_email_auth_key');
		}
	...
	function procMemberSendVerificationSMS()
	{
		...
		{
			$phone_country_calling_code = preg_replace('/[^0-9]/', '', Zittme\Framework\i18n::getCallingCodeByCountryCode($phone_country));
		...
		if ($phone_country === 'KOR' && !Zittme\Framework\Korea::isValidPhoneNumber($phone_number ?? ''))
		{
			return new BaseObject(-1, 'msg_invalid_phone_number');
		}
	...
	function procMemberConfirmVerificationSMS()
	{
		...
		{
			throw new Zittme\Framework\Exception('verify_by_sms_code_incorrect');
		}
		...
		if (isset($_SESSION['verify_by_sms']['count']) && $_SESSION['verify_by_sms']['count'] >= 10)
		{
			...
			throw new Zittme\Framework\Exception('verify_by_sms_code_too_many_tries');
		}
		if (isset($_SESSION['verify_by_sms']['time']) && $_SESSION['verify_by_sms']['time'] < time() - 600)
		{
			unset($_SESSION['verify_by_sms']);
			throw new Zittme\Framework\Exception('verify_by_sms_code_expired');
		}
	...
	**/
	...
	public static function clearMemberCache($member_srl)
	{
		...
		Zittme\Framework\Cache::delete("member:member_info:$member_srl");
		Zittme\Framework\Cache::delete("member:member_groups:$member_srl");
		Zittme\Framework\Cache::delete("site_and_module:accessible_modules:$member_srl");
