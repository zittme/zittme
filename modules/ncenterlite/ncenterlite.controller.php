...
class NcenterliteController extends Ncenterlite
{
	...
	function procNcenterliteUserConfig()
	{
		...
		if(!Zittme\Framework\Session::isMember())
		{
			throw new Zittme\Framework\Exceptions\MustLogin;
		}
		if($config->user_notify_setting != 'Y')
		{
			throw new Zittme\Framework\Exception('msg_not_use_user_setting');
		}
		...
		{
			throw new Zittme\Framework\Exception('ncenterlite_stop_no_permission_other_user_settings');
		}
		...
		Zittme\Framework\Cache::delete('ncenterlite:user_config:' . $member_srl);
	...
	function procNcenterliteInsertUnsubscribe()
	{
		...
		{
			throw new Zittme\Framework\Exception('msg_unsubscribe_block_not_support');
		}
		...
		if(!$userBlockData && $obj->unsubscribe_srl)
		{
			...
			{
				throw new Zittme\Framework\Exception('ncenterlite_stop_no_permission_other_user_block_settings');
			}
		...
		else
		{
			...
			{
				throw new Zittme\Framework\Exception('msg_unsubscribe_not_in_list');
			}
	...
	function triggerBeforeDisplay(&$output_display)
	{
		...
		{
			Zittme\Framework\Cookie::remove('_ncenterlite_hide_id');
		}
	...
	public function removeFlagFile($member_srl = null)
	{
		...
		$cache_key = sprintf('ncenterlite:notify_list:%d', $member_srl);
		Zittme\Framework\Cache::delete($cache_key);
	...
	function sendPushMessage($args)
	{
		...
		{
			$target_url = Zittme\Framework\URL::getCurrentDomainUrl($target_url);
		}
		...
		if (!isset($args->extra_data) || !$args->extra_data)
		{
			...
			if ($args->target_member_srl > 0)
			{
				...
				{
					$args->extra_data['profile_image'] = Zittme\Framework\URL::getCurrentDomainUrl($profile_image->src);
				}
			}
		...
		$oPush = new \Zittme\Framework\Push();
	...
	function sendSmsMessage($args)
	{
		...
		if($config->variable_name)
		{
			...
			$phone_format = Zittme\Framework\Korea::isValidPhoneNumber($phone_number);
	...
	**/
	...
	{
		if(!Zittme\Framework\Session::isMember()) return;
