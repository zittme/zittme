...
class NcenterliteModel extends Ncenterlite
{
	...
	public static function getNotifyTypes()
	{
		...
		$custom_types = Zittme\Framework\Cache::get('ncenterlite:notify_types');
		if (!$custom_types)
		{
			$custom_types = executeQueryArray('ncenterlite.getNotifyType', [])->data;
			Zittme\Framework\Cache::set('ncenterlite:notify_types', $custom_types, 0, true);
		}
	...
	function insertNotifyType($args)
	{
		...
		Zittme\Framework\Cache::delete('ncenterlite:notify_types');
		return $output;
	}
	...
	public static function getUserConfig($member_srl = null)
	{
		...
		$config = Zittme\Framework\Cache::get('ncenterlite:user_config:' . $member_srl);
		if ($config !== null)
		{
			return $config;
		}
		...
		Zittme\Framework\Cache::set('ncenterlite:user_config:' . $member_srl, $config);
		return $config;
	}
	...
	function _getMyNotifyList($member_srl = null, $page = 1, $readed = 'N')
	{
		...
		if ($page <= 1 && $readed == 'N')
		{
			$output = Zittme\Framework\Cache::get($cache_key);
			if($output !== null)
			{
				return $output;
			}
		...
		if($page <= 1 && $readed == 'N' && FileHandler::exists($flag_path))
		{
			...
			$deleteOutput = Zittme\Framework\Storage::readPHPData($deleteFlagPath);
			if($deleteOutput !== false)
			{
				...
				{
					$output = Zittme\Framework\Storage::readPHPData($flag_path);
					if($output !== false)
					{
	...
	function _getNewCount($member_srl = null)
	{
		...
		elseif (Zittme\Framework\Cache::getDriverName() !== 'dummy')
		{
