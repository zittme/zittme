@if ($type === 'country')
	@php
		...
		$lang_sort = $lang_type === 'ko' ? Zittme\Framework\i18n::SORT_NAME_KOREAN : Zittme\Framework\i18n::SORT_NAME_ENGLISH;
		$countries = Zittme\Framework\i18n::listCountries($lang_sort);
	@endphp
...
@elseif ($type === 'language')
	@php
		$enabled_languages = Zittme\Framework\Config::get('locale.enabled_lang');
		$supported_languages = Zittme\Framework\Lang::getSupportedList();
	@endphp
	<select name="{{ $input_name }}"
...
@elseif ($type === 'timezone')
	@php
		$timezones = Zittme\Framework\DateTime::getTimezoneList();
	@endphp
	<select name="{{ $input_name }}"
