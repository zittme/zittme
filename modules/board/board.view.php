...
class BoardView extends Board
{
	...
	public function init()
	{
		 ...
		 */
		// 좁은 화면 여부로 판단한다. 반응형 뷰에서는 PC 스킨을 그대로 쓰므로
		// Context 의 'm'(기기 모드)은 항상 0 이고, 그것만 보면 모바일 설정값이
		// 영원히 적용되지 않는다. 반응형이 꺼져 있으면 기존과 동일하게 동작한다.
		$m = Mobile::isNarrowScreen();
	...
	public function dispBoardContent()
	{
		...
		$board_features = Zittme\Modules\Board\Models\Features::fromModuleInfo($this->module_info, $document_module_srl);
	...
	public function dispBoardDelete()
	{
		...
		if ($this->module_info->protect_admin_content_delete !== 'N')
		{
			...
			if(isset($member_info->is_admin) && $member_info->is_admin == 'Y' && $this->user->is_admin != 'Y')
			{
				throw new Zittme\Framework\Exception('document.msg_document_is_admin_not_permitted');
			}
		}
	...
	public function dispBoardDeleteComment()
	{
		...
		if ($this->module_info->protect_admin_content_delete !== 'N' && $logged_info->is_admin !== 'Y' && $logged_info->member_srl !== $oComment->member_srl)
		{
			...
			if($member_info->is_admin === 'Y')
			{
				throw new Zittme\Framework\Exception('msg_admin_comment_no_delete');
			}
		}
	...
	public function dispBoardVoteLog()
	{
		...
		{
			throw new Zittme\Framework\Exception('msg_not_target');
		}

		$features = Zittme\Modules\Board\Models\Features::fromModuleInfo($this->module_info);
