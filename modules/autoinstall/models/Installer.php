	...
	public static function installPackage(Package $package, string $mode = 'install'): BaseObject
	{
		...
		for ($i = 0; $i < $zip->numFiles; $i++)
		{
			...
			else
			{
				...
				// No existing parent below the base directory (e.g. installing the
				// first theme when themes/ does not exist yet). Check the base itself.
				if (!$found_parent)
				{
					$permissions = Storage::isWritable(\RX_BASEDIR);
					$last_checked_path = \RX_BASEDIR;
				}
			}
