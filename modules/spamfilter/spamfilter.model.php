...
class SpamfilterModel extends Spamfilter
{
	...
	{
		$ip_list = Zittme\Framework\Cache::get('spamfilter:denied_ip_list');
		if ($ip_list === null)
		{
			$ip_list = self::getDeniedIPList();
			Zittme\Framework\Cache::set('spamfilter:denied_ip_list', $ip_list);
		}
		if (!count($ip_list))
		{
		...
		foreach ($ip_list as $ip_range)
		{
			...
			if (Zittme\Framework\Filters\IpFilter::inRange(\RX_CLIENT_IP, $ip_range->ipaddress))
			{
				$args = new stdClass();
	...
	{
		$word_list = Zittme\Framework\Cache::get('spamfilter:denied_word_list');
		if ($word_list === null)
		{
			$word_list = self::getDeniedWordList();
			Zittme\Framework\Cache::set('spamfilter:denied_word_list', $word_list);
		}
		if (!count($word_list))
		{
			return new BaseObject();
	...
	public static function checkLimited($isMessage = FALSE)
	{
		...
		{
			if (Zittme\Framework\Filters\IpFilter::inRanges(\RX_CLIENT_IP, $config->except_ip))
			{
				return new BaseObject();
			}
		}
	...
	public static function getCaptcha($target_action = null)
	{
		...
		$captcha_class = 'Zittme\\Modules\\Spamfilter\\Captcha\\' . $config->captcha->type;
