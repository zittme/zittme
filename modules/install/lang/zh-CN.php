...
$lang->msg_ftp_installed_realpath = 'Zittme绝对路径';
$lang->msg_ftp_installed_ftp_realpath = 'FTP绝对路径设置';
$lang->msg_possible_only_file = '请上传文件.';
$lang->about_nginx_rewrite = 'nginx 使用短地址需要配置 rewrite。点击下方按钮即可复制为本服务器生成的配置及应用命令。';

// nginx rewrite helper layer (Zittme)
$lang->nginx_helper_open = '查看 nginx 设置方法';
$lang->nginx_helper_title = 'nginx 短地址(rewrite)设置';
$lang->nginx_helper_requirements = '前提条件: 仅适用于拥有 root SSH 权限的自管服务器(VPS/独立服务器)。共享主机请联系主机商。以下内容根据当前服务器环境自动生成。';
$lang->nginx_helper_commands_label = '通过 SSH 执行的命令';
$lang->nginx_helper_config_label = '生成的 nginx 配置';
$lang->nginx_helper_copy = '复制';
$lang->nginx_helper_copied = '已复制!';
$lang->nginx_helper_after = '应用后刷新此页面，mod_rewrite 项将显示 OK。不配置也可以继续安装。';
$lang->enviroment_gather = '同意收集安装环境信息';
$lang->input_dbinfo_by_dbtype = '输入 %s 信息';
$lang->install_condition_enable = '您的服务器兼容 Zittme。';
$lang->install_condition_disable = '您的服务器不支持 Zittme。';
$lang->install_details = '详细';
$lang->install_simply = '简略';
$lang->advanced_setup = '高级设置';
$lang->install_ftp_reason = '需要 FTP 信息的原因。';
$lang->install_checklist_title['db_support'] = '数据库支持';
$lang->install_checklist_desc['php_version'] = '[必需] Zittme 仅支持 PHP %s 及以上版本';
$lang->install_checklist_desc['php_version_warning'] = '[建议] Zittme 建议使用 PHP %s 及以上版本';
$lang->install_checklist_desc['free_domain_warning'] = '如果您使用的是主机商提供的免费二级域名（例如 ID.hosting-company.com），<br />请清除浏览器 Cookie 后返回上一步重新开始安装。';
$lang->install_notandum = '安装完成后，您可以在管理模块中修改这些信息。';
$lang->db_info_desc = '请向服务器管理员确认数据库服务器信息。';
$lang->db_prefix_desc = '您可以自定义数据表前缀。建议仅使用英文小写字母。';
$lang->checking_rewrite = '正在检查是否可以使用“短网址”功能……';
$lang->disable_rewrite = '无法使用“短网址”功能。请向服务器管理员确认是否支持 mod_rewrite 模块。';
$lang->disable_rewrite_can_proceed = '即使无法使用“短网址”功能，也不影响 Zittme 的安装和使用。';
$lang->use_sitelock = '站点锁定';
$lang->sitelock_options['none'] = '不使用';
$lang->sitelock_options['lock'] = '安装后锁定';
$lang->about_sitelock_after_install = '锁定站点后，仅当前 IP 段（%s）可以访问您的站点。';
$lang->msg_db_checking = '正在检查……';
$lang->msg_installing = '正在安装……';
$lang->msg_dbroot_disallowed = '不允许使用数据库 root 账号进行安装。请创建并使用非 root 的普通数据库账号。';
$lang->msg_create_table_failed = '创建数据表失败。';
$lang->msg_table_already_exists = '数据表已存在。请尝试使用其他表前缀。';
$lang->db_config_php_validation = 'db.config.php 有效性检查';
