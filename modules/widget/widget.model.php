...
class WidgetModel extends Widget
{
	...
	{
		// '테마|@|위젯' 이면 테마 폴더 안을 가리킨다.
		$theme_parts = Zittme\Framework\Theme::split($widget_name);
		if($theme_parts)
		{
			$path = sprintf('./%s%s/widgets/%s/',
				Zittme\Framework\Theme::BASE_DIR, $theme_parts['theme'], $theme_parts['name']);
		}
		else
		{
	...
	public static function getWidgetInfo($widget)
	{
		...
		$widget_info = Zittme\Framework\Cache::get($cache_key);
		if ($widget_info)
		{
		...
		$widget_info = Zittme\Framework\Parsers\WidgetInfoParser::loadXML($xml_file, $widget);
		if (!$widget_info)
		{
			return;
		}

		Zittme\Framework\Cache::set($cache_key, $widget_info);
		$GLOBALS['__widget_info__'][$widget][$xml_mtime] = $widget_info;
		return $widget_info;
	}
	...
	public static function getWidgetStyleInfo($widgetStyle)
	{
		...
		$widgetStyle_info = Zittme\Framework\Cache::get($cache_key);
		if ($widgetStyle_info)
		{
		...
		// Parse the XML file and store the result in the cache.
		$widgetStyle_info = Zittme\Framework\Parsers\WidgetStyleInfoParser::loadXML($xml_file, $widgetStyle);
		if (!$widgetStyle_info)
		{
			return;
		}

		Zittme\Framework\Cache::set($cache_key, $widgetStyle_info);
		$GLOBALS['__widgetstyle_info__'][$widgetStyle][$xml_mtime] = $widgetStyle_info;
		return $widgetStyle_info;
	}
}
