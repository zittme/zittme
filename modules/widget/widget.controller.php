...
class WidgetController extends Widget
{
	...
	function procWidgetGetColorsetList()
	{
		...
		$path = WidgetModel::getWidgetPath($widget) ?: sprintf('./widgets/%s/', $widget);
	...
	function procWidgetGenerateCode()
	{
		...
		if (!Context::get('skin'))
		{
			throw new Zittme\Framework\Exception('msg_widget_skin_is_null');
		}
	...
	function procWidgetCopyDocument()
	{
		...
		$columnList = array('module_srl', 'module');
		$page_info = ModuleModel::getModuleInfoByModuleSrl($module_srl, $columnList);
		if(!$page_info->module_srl || $page_info->module != 'page') throw new Zittme\Framework\Exceptions\InvalidRequest;
	...
	function getCache($widget, $args, $lang_type = null, $ignore_cache = false, $override_sequence = false)
	{
		...
		$cache_data = Zittme\Framework\Cache::get($cache_key);
		...
		Zittme\Framework\Cache::set($cache_key, $cache_data, $widget_cache, true);
	...
	function execute($widget, $args, $javascript_mode = false, $escaped = true)
	{
		...
		if(!$javascript_mode)
		{
			...
			switch($widget)
			{
				...
				case 'widgetContent' :
					...
					$widget_content_header = sprintf('<div class="Zittme_content xe_content xe-widget-wrapper ' . ($args->css_class ?? '') . '" %sstyle="%s"><div style="%s">', $args->id ?? '', $style, $inner_style);
			...
			switch($widget)
			{
				...
				case 'widgetContent' :
					...
					$widget_content_header = vsprintf(
						'<div class="Zittme_content xe_content widgetOutput ' . ($args->css_class ?? '') . '" widgetstyle="%s" style="%s" widget_padding_left="%s" widget_padding_right="%s" widget_padding_top="%s" widget_padding_bottom="%s" widget="widgetContent" document_srl="%d" %s>'.
		...
		{
			Zittme\Framework\Debug::addWidget(array(
				'name' => $widget,
	...
	function compileWidgetStyle($widgetStyle,$widget,$widget_content_body, $args, $javascript_mode)
	{
		...
		$oTemplate = Zittme\Framework\Template::getInstance();
	...
	function arrangeWidgetVars($widget, $request_vars, &$vars)
	{
		...
		{
			$lang_type = Context::getLangType();
			Zittme\Framework\Cache::delete('widget_cache:' . $vars->widget_sequence . ':' . $lang_type);
		}
