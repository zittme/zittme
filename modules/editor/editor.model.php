...
class EditorModel extends Editor
{
	...
	public static function getEditor($upload_target_srl = 0, $option = null)
	{
		...
		if ($option->editor_skin === 'zitteditor' && !in_array($option->editor_colorset, array('moono', 'moono-dark', 'moono-lisa')))
		{
			$option->editor_colorset = 'moono-lisa';
		}
		// 반응형 뷰 사이트는 리디자인 에디터(zittEditor)를 기본으로 쓴다.
		// 명시적으로 다른 스킨을 고른 경우는 존중하고, 구 기본값(ckeditor)일 때만 승격한다.
		if ($option->editor_skin === 'ckeditor' && Mobile::isResponsiveView()
			&& file_exists('./modules/editor/skins/zitteditor/editor.html'))
		{
			$option->editor_skin = 'zitteditor';
		}
		...
		if($option->allow_fileupload)
		{
			...
			$browser = Zittme\Framework\UA::getBrowserInfo();
	...
	public static function getModuleEditor($type, $module_srl, $upload_target_srl, $primary_key_name, $content_key_name)
	{
		...
		// Check mobile status. isNarrowScreen() follows the actual screen width when
		// the responsive view is on, and falls back to isFromMobilePhone() otherwise,
		// so this behaves exactly as before for sites that do not use it.
		$is_mobile = Mobile::isNarrowScreen() || \Zittme\Framework\UA::isMobile();
		...
		if($type == 'document')
		{
			...
			// Under the responsive view the browser may switch between the two sets
			// without a reload, so hand the skin both. They have to be captured before
			// the block below overwrites the desktop values with the mobile ones.
			if (Mobile::isResponsiveView())
			{
				$option->responsive_settings = [
					'wide' => [
						'height' => $option->editor_height ?? 100,
						'toolbar' => $option->editor_toolbar ?? 'default',
						'hide_toolbar' => $option->editor_toolbar_hide ?? false,
					],
					'narrow' => [
						'height' => $option->mobile_editor_height ?: ($option->editor_height ?? 100),
						'toolbar' => $option->mobile_editor_toolbar ?: 'default',
						'hide_toolbar' => $option->mobile_editor_toolbar_hide ?? false,
					],
				];
			}
			...
			if ($is_mobile)
			{
				// The responsive view keeps one editor and only adjusts it, so the skin
				// is deliberately left alone: swapping it would mean a different editor
				// component, which cannot be switched in place without losing the draft.
				// Everything below is toolbar/size/appearance and can change live.
				if (!Mobile::isResponsiveView())
				{
		...
		{
			$logged_info = new Zittme\Framework\Helpers\SessionHelper();
			$group_list = array();
		}
	...
	public static function getSavedDoc($upload_target_srl)
	{
		...
		{
			$saved_doc->certify_key = Zittme\Framework\Security::getRandom(32);
	...
	public static function getComponentList($filter_enabled = true, $site_srl = 0, $from_db = false)
	{
		...
		$component_list = $from_db ? null : Zittme\Framework\Cache::get($cache_key);
		if (!$component_list)
		{
			$oEditorController = getController('editor');
	...
	public static function getComponentXmlInfo($component)
	{
		...
		// Parse XML file
		$info = Zittme\Framework\Parsers\EditorComponentParser::loadXML($xml_file, $component, $lang_type);
	...
	public static function converter($obj, $type = null)
	{
		...
		if ($converter)
		{
			...
			{
				$obj->content = Zittme\Framework\Formatter::text2html($obj->content);
			}
			elseif ($converter == 'markdown2html')
			{
				$obj->content = Zittme\Framework\Formatter::markdown2html($obj->content);
			}
			elseif ($converter == 'bbcode')
			{
				$obj->content = Zittme\Framework\Formatter::bbcode($obj->content);
			}
