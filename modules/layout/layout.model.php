...
class LayoutModel extends Layout
{
	...
	{
		// '테마|@|레이아웃' 이면 테마 폴더 안을 본다.
		$theme_parts = Zittme\Framework\Theme::split($layout);
		if($theme_parts)
		{
			$path = rtrim(Zittme\Framework\Theme::getLayoutPath($theme_parts['theme'], $theme_parts['name'], $layoutType), '/');
		}
		elseif($layoutType == 'P')
		{
	...
	public static function getLayout($layout_srl, $use_cache = true)
	{
		...
		$layout_info = Zittme\Framework\Cache::get("layout:$layout_srl");
		...
		// Store in cache
		Zittme\Framework\Cache::set("layout:$layout_srl", $layout_info);
		return $layout_info;
	}
	...
	{
		// '테마|@|레이아웃' 이면 테마 폴더 안을 가리킨다.
		$theme_parts = Zittme\Framework\Theme::split($layout_name);
		if($theme_parts)
		{
			$dir = ($layout_type === 'M') ? 'm.layouts' : 'layouts';
			$class_path = sprintf('./%s%s/%s/%s/',
				Zittme\Framework\Theme::BASE_DIR, $theme_parts['theme'], $dir, $theme_parts['name']);
		}
	...
	public static function _getInstalledLayoutDirectories($layoutType = 'P')
	{
		...
		// 테마가 가진 레이아웃도 함께 보여 준다. 이름이 '테마|@|레이아웃' 이라
		// 일반 레이아웃과 섞여도 서로 덮지 않는다.
		foreach(array_keys(Zittme\Framework\Theme::getLayouts($layoutType)) as $themeLayout)
		{
			$searchedList[] = $themeLayout;
		}
	...
	/**
	 * Filter repeat-field items for front display: keep only visible items whose
	 * schedule window (if any) covers now. Layout templates call this so they can
	 * simply foreach the result. (docs/LAYOUT-REPEAT-FIELDS.md)
	 *
	 * @param array|object|null $items decoded repeat value (extra_var->{name}->value)
	 * @return array visible items in order
	 */
	public static function getVisibleItems($items)
	{
		if (!is_array($items))
		{
			$items = is_object($items) ? (array)$items : [];
		}
		$now = date('YmdHis');
		$result = [];
		...
		{
			$item = (object)$item;
			if (isset($item->_visible) && !$item->_visible)
			{
				continue;
			}
			if (!empty($item->_start_date) && $now < $item->_start_date)
			{
				continue;
			}
			if (!empty($item->_end_date) && $now > $item->_end_date)
			{
				continue;
			}
			$result[] = $item;
	...
	public static function getLayoutInfo($layout, $info = null, $layout_type = "P")
	{
		...
		$xml_info = Zittme\Framework\Parsers\LayoutInfoParser::loadXML($xml_file, $layout, $layout_path);
		if (!$xml_info)
		{
			return;
		}
		...
		foreach ($xml_info->extra_var ?: [] as $key => $value)
		{
			if (isset($vars->{$key}))
			{
				// Repeat fields store a JSON array of item objects — decode it so
				// the admin form and templates receive an array. (docs/LAYOUT-REPEAT-FIELDS.md)
			...
			elseif (isset($value->type) && $value->type === 'repeat')
			{
				$value->value = [];
			}
		}
		...
		Zittme\Framework\Storage::writePHPData($cache_file, $xml_info, null, false);
		return $xml_info;
	}
