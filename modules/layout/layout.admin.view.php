...
class LayoutAdminView extends Layout
{
	...
	function dispLayoutAdminInstanceList()
	{
		...
		if(!$layout) throw new Zittme\Framework\Exceptions\InvalidRequest;

		$oLayoutModel = getModel('layout');
		$layout_info = $oLayoutModel->getLayoutInfo($layout, null, $type);
		if(!$layout_info) throw new Zittme\Framework\Exceptions\InvalidRequest;
	...
	function dispLayoutAdminInsert()
	{
		...
		// Get layout info
		$layout = Context::get('layout');
		if($layout == 'faceoff') throw new Zittme\Framework\Exception('not supported');
	...
	function dispLayoutAdminModify()
	{
		...
		Context::loadFile(array('./modules/layout/tpl/js/layout_repeat.js', 'body'));
		Context::loadFile('./modules/layout/tpl/css/layout_repeat.css');
	...
	/**
	 * Chrome-less layout settings form — only the settings fields, no admin shell.
	 * Loaded inside the front admin bar's slim left modal. (docs/FRONT-ADMIN-BAR.md)
	 */
	function dispLayoutAdminModifyForm()
	{
		$logged_info = Context::get('logged_info');
		if(!$logged_info || ($logged_info->is_admin ?? 'N') !== 'Y')
		{
			throw new Zittme\Framework\Exception('msg_not_permitted');
		}
		getAdminModel('layout')->setLayoutAdminSetInfoView();
		Context::set('is_sitemap', '0');
		Context::set('zab_bare', true);      // slim modal render — adds body padding
		Context::loadFile(array('./modules/layout/tpl/js/layout_modify.js', 'body'));
		Context::loadFile(array('./modules/layout/tpl/js/layout_repeat.js', 'body'));
		Context::loadFile('./modules/layout/tpl/css/layout_repeat.css');
		$this->setLayoutFile('');            // no site/admin layout — just the form
		$this->setTemplateFile('layout_info_view');
	}
	...
	function dispLayoutAdminPreview()
	{
		...
		if(!$layout_srl || !$code) throw new Zittme\Framework\Exceptions\InvalidRequest;
