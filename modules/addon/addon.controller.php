...
class addonController extends addon
{
	...
	function makeCacheFile($site_srl = 0, $type = "pc", $gtype = 'site')
	{
		...
		foreach($addon_list as $addon => $val)
		{
			...
			$buff[] = '  if (class_exists("Zittme\\\\Framework\\\\Debug") && Zittme\\Framework\\Debug::isEnabledForCurrentUser()):';
			$buff[] = '    Zittme\\Framework\\Debug::addTrigger(array(';
	...
	function doSetup($addon, $extra_vars, $site_srl = 0, $gtype = 'site')
	{
		...
		Zittme\Framework\Cache::delete(sprintf('addonConfig:%s:%s', $args->addon, 'any'));
		Zittme\Framework\Cache::delete(sprintf('addonConfig:%s:%s', $args->addon, 'pc'));
		Zittme\Framework\Cache::delete(sprintf('addonConfig:%s:%s', $args->addon, 'mobile'));
		return $output;
	}
