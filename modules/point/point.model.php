...
class PointModel extends Point
{
	...
	public static function getPoint($member_srl, $from_db = false, &$exists = null)
	{
		...
		$use_cache = Zittme\Framework\Cache::getDriverName() !== 'dummy';
		$cache_key = sprintf('member:point:%d', $member_srl);
		if (!$from_db && $use_cache)
		{
			$point = Zittme\Framework\Cache::get($cache_key);
			if ($point !== null)
			{
				$exists = true;
		...
		if (!$from_db && !$use_cache)
		{
			...
			{
				$point = trim(Zittme\Framework\Storage::read($cache_filename));
				if ($point !== '')
				{
					$exists = true;
		...
		if ($use_cache)
		{
			Zittme\Framework\Cache::set($cache_key, $point);
		}
		else
		{
			Zittme\Framework\Storage::write($cache_filename, $point);
		}
