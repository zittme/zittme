...
class DocumentModel extends Document
{
	...
	public static function getExtraKeys($module_srl)
	{
		...
		if(!isset($GLOBALS['XE_EXTRA_KEYS'][$module_srl]))
		{
			$keys = Zittme\Framework\Cache::get("site_and_module:module_document_extra_keys:$module_srl");
			$oExtraVar = new Zittme\Modules\Extravar\Models\ValueCollection($module_srl);
			...
			if($keys === null)
			{
				...
				Zittme\Framework\Cache::set("site_and_module:module_document_extra_keys:$module_srl", $keys, 0, true);
			}
	...
	public function getDocumentMenu()
	{
		...
		if(!$module_srl) throw new Zittme\Framework\Exceptions\InvalidRequest;
	...
	public function getDocumentVotedMemberList()
	{
		...
		if($point == -1)
		{
			if($document_config->use_vote_down!='S') throw new Zittme\Framework\Exceptions\FeatureDisabled;
			$args->below_point = 0;
		}
		else
		{
			if($document_config->use_vote_up!='S') throw new Zittme\Framework\Exceptions\FeatureDisabled;
			$args->more_point = 0;
		}
