...
class DocumentItem extends BaseObject
{
	...
	function _loadFromDB($load_extra_vars = true, $reload_counts = true)
	{
		...
		$document_item = Zittme\Framework\Cache::get($cache_key);
		if($document_item)
		{
		...
		if(!$document_item)
		{
			...
			if($document_item)
			{
				Zittme\Framework\Cache::set($cache_key, $document_item);
			}
		}
	...
	function getContent($add_popup_menu = true, $add_content_info = true, $resource_realpath = false, $add_xe_content_class = true, $stripEmbedTagException = false)
	{
		...
		if($add_content_info)
		{
			...
			$content = sprintf(
				'<!--BeforeDocument(%d,%d)--><div class="document_%d_%d Zittme_content xe_content">%s</div><!--AfterDocument(%d,%d)-->',
		...
		{
			$content = sprintf('<div class="Zittme_content xe_content">%s</div>', $content);
		}
	...
	{
		return Zittme\Framework\DateTime::formatTimestampForCurrentUser($format, $this->getRegdateTime());
	}
	...
	{
		return Zittme\Framework\DateTime::formatTimestampForCurrentUser($format, $this->getUpdateTime());
	}
		...
		if(!$source_file && $config->thumbnail_target !== 'attachment')
		{
			...
			foreach($matches as $match)
			{
				...
				else
				{
					...
					$tmp_file = sprintf('./files/cache/tmp/%s', Zittme\Framework\Security::getRandom(32));
					if (!Zittme\Framework\Storage::exists(\RX_BASEDIR . 'files/cache/tmp'))
					{
						Zittme\Framework\Storage::createDirectory(\RX_BASEDIR . 'files/cache/tmp');
					}
					if (!Zittme\Framework\Storage::exists(\RX_BASEDIR . 'files/cache/tmp/.htaccess'))
					{
						Zittme\Framework\Storage::protectDirectory(\RX_BASEDIR . 'files/cache/tmp');
					}
					FileHandler::getRemoteFile($target_src, $tmp_file);
					if (!Zittme\Framework\Storage::exists($tmp_file))
					{
						continue;
					}
					else
					{
						if ($is_img = @getimagesize($tmp_file))
						{
							...
							{
								Zittme\Framework\Storage::delete($tmp_file);
								continue;
							}
						}
						else
