...
class MenuAdminModel extends Menu
{
	...
	function getMenuAdminInstalledMenuType()
	{
		...
		// Design panel skin rows. Modules that hide themselves from the menu
		// creation list (commerce, reservation) must still be listed here.
		$this->add('design_types', $this->getModuleListInSitemap(0, false));
	 ...
	 * @return array
	 */
	function getModuleListInSitemap($site_srl = 0, $apply_trigger = true)
	{
		$oModuleModel = getModel('module');
		...
		// after trigger. Skipped for the design panel list, where modules that
		// exclude themselves from menu creation should still appear.
		if($apply_trigger)
		{
	...
	public function getMenuAdminSiteMap()
	{
		...
		else
		{
			...
			if(is_array($menuListFromDB))
			{
				...
				{
					if($value->title === Zittme\Modules\Admin\Models\AdminMenu::getAdminMenuName()) unset($output[$key]);
					else
					{
						unset($menu);
						unset($menuItems);
						...
						// 홈이 포함된 사이트맵인지 표시만 한다. 예전에는 이 사이트맵을 무조건
						// 맨 앞으로($menuList[-1]) 끌어올려 순서 이동을 막았으나, 이제는
						// listorder 순서를 그대로 따르고 홈 여부는 별표로만 알린다.
						$menuItems->isHomeSitemap = ($value->menu_srl == $this->menuSrlWithinHome);
						$menuList[] = $menuItems;
					}
				}
			}
