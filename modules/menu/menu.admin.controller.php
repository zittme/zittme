...
class MenuAdminController extends Menu
{
	...
	function updateLinkModule($moduleInfos, $menuSrl)
	{
		...
		Zittme\Framework\Cache::clearGroup('site_and_module');
		...
		if ($output->data)
		{
			...
			if (count($module_srls))
			{
				...
				if ($layout_M)
				{
					...
					// 모바일 레이아웃을 지정하면 모바일 뷰를 켜 주는 것이 기존 동작이다.
					// 다만 사이트가 반응형이면 모바일 레이아웃 자체가 쓰이지 않으므로,
					// 각 모듈의 뷰 모드를 건드리지 않고 레이아웃만 저장한다.
					// (var 가 설정되지 않은 컬럼은 UPDATE 에서 제외된다)
	...
	function procMenuAdminDelete()
	{
		...
		if($menuInfo->title == $oAdmin->getAdminMenuName())
			throw new Zittme\Framework\Exception('msg_adminmenu_cannot_delete');
		...
		if($isStartmenuInclude)
		{
			throw new Zittme\Framework\Exception('msg_cannot_delete_homemenu');
		}
	...
	private function _insertMenu(&$request, $isProc)
	{
		...
		if($isProc)
		{
			...
			if(!$result->toBool())
			{
				throw new Zittme\Framework\Exception($result->message);
			}
		}
	...
	private function _insertModule(&$request, &$args)
	{
		...
		{
			throw new Zittme\Framework\Exception('msg_module_name_exists');
		}
	 ...
	 */
	/**
	 * 대메뉴(사이트맵 단위)의 표시 순서를 바꾼다.
	 *
	 * 이 순서는 관리자 사이트맵 트리에서 어느 사이트맵이 먼저 오는지를 정하는 값일 뿐,
	 * 방문자 화면(GNB 등)에는 영향을 주지 않는다. menu 테이블은 listorder DESC 로
	 * 정렬되므로(getMenus.xml), 넘어온 순서대로 큰 값부터 내려 매긴다.
	 */
	function procMenuAdminReorderMenu()
	{
		$menu_srls = Context::get('menu_srls');
		if(is_string($menu_srls))
		{
			$menu_srls = array_filter(array_map('trim', explode(',', $menu_srls)), 'strlen');
		}
		if(!is_array($menu_srls) || !count($menu_srls))
		{
			throw new Zittme\Framework\Exceptions\InvalidRequest;
		}
		...
		// 요청에 담긴 메뉴들이 실제로 존재하는지 확인한다. 없는 srl 로 순서를 쓰지 않게.
		$valid = array();
		foreach($menu_srls as $menu_srl)
		{
			$menu_srl = (int)$menu_srl;
			$menuInfo = $oMenuAdminModel->getMenu($menu_srl);
			if($menuInfo->menu_srl)
			{
				$valid[] = $menu_srl;
			}
		}
		...
		// 앞에 온 것이 위(먼저)로 오도록 큰 listorder 부터 내려 준다.
		$listorder = count($valid);
		foreach($valid as $menu_srl)
		{
			$args = new stdClass;
			$args->menu_srl = $menu_srl;
			$args->listorder = $listorder--;
			executeQuery('menu.updateMenuListorder', $args);
		}
	...
	function procMenuAdminMoveItem()
	{
		...
		if(!$mode || !$parent_srl || !$target_srl) throw new Zittme\Framework\Exceptions\InvalidRequest;
		...
		{
			throw new Zittme\Framework\Exception('msg_empty_menu_item');
		}
	...
	function moveMenuItem($menu_srl, $parent_srl, $source_srl, $target_srl, $mode, $isShortcut='Y', $url=NULL)
	{
		...
		if($target_item->menu_item_srl != $target_srl) throw new Zittme\Framework\Exceptions\InvalidRequest;
		if($target_srl == $parent_srl) throw new Zittme\Framework\Exceptions\InvalidRequest;
		...
		if($mode == 'move')
		{
			...
			if($source_srl)
			{
				...
				if($source_item->menu_item_srl != $source_srl) throw new Zittme\Framework\Exceptions\InvalidRequest;
	...
	function getXmlTree($source_node, $tree, $site_srl, $domain)
	{
		...
		foreach($source_node as $menu_item_srl => $node)
		{
			...
			$url = escape($node->url);
			$icon = Zittme\Framework\Filters\HTMLFilter::clean($node->icon ?? '', true);
			$class = Zittme\Framework\Filters\HTMLFilter::clean($node->class ?? '', true);
			$desc = Zittme\Framework\Filters\HTMLFilter::clean($node->desc ?? '', true);
	...
	function getPhpCacheCode($source_node, $tree, $site_srl, $domain)
	{
		...
		foreach($source_node as $menu_item_srl => $node)
		{
			...
			foreach($names as $key => $val)
			{
				...
				{
					$name_arr_str .= sprintf('"%s"=>%s,', $key, var_export(Zittme\Framework\Filters\HTMLFilter::clean($val, true), true));
				}
			}
