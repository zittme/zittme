...
class PageAdminController extends Page
{
	...
	function procPageAdminInsert()
	{
		...
		$args->use_mobile = Mobile::sanitizeViewMode($args->use_mobile ?? '');
	...
	function procPageAdminRemoveWidgetCache()
	{
		...
		Zittme\Framework\Cache::delete('site_and_module:mid_info:' . $module_info->module_srl);
	}
	...
	protected static function _isAllowedExternalPath(string $path): bool
	{
		...
		if (preg_match('!(?:image|audio|video)/!i', Zittme\Framework\MIME::getTypeByFilename($path)))
		{
			return false;
		}
