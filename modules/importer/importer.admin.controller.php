...
class importerAdminController extends importer
{
	...
	function procImporterAdminImport()
	{
		...
		if(!file_exists($index_file)) throw new Zittme\Framework\Exception('msg_invalid_xml_file');

		switch($type)
		{
			case 'ttxml' :
				if(!$target_module) throw new Zittme\Framework\Exceptions\InvalidRequest;
	...
	function importMember($key, $cur, $index_file)
	{
		...
		for($idx=$cur;$idx<$cur+$this->unit_count;$idx++)
		{
			...
			$xmlObj = Zittme\Framework\Parsers\XEXMLParser::loadXMLFile($target_file);
			...
			if($output->toBool() && !($obj->password))
			{
				...
				$oMail->setContent("Dear $obj->user_name, <br /><br />
						We recently migrated our site to Zittme. Since you password was encrypted we could not migrate it too, so please reset it by following this link:
	...
	function importModule($key, $cur, $index_file, $module_srl)
	{
		...
		if(file_exists($category_file))
		{
			...
			$xmlDoc = Zittme\Framework\Parsers\XEXMLParser::loadXMLFile($category_file);
		...
		for($idx=$cur;$idx<$cur+$this->unit_count;$idx++)
		{
			...
			$xmlDoc = Zittme\Framework\Parsers\XEXMLParser::loadXMLString(implode('', $buff));
	...
	function importTrackbacks($fp, $module_srl, $document_srl)
	{
		...
		while(!feof($fp))
		{
			...
			{
				$xmlDoc = Zittme\Framework\Parsers\XEXMLParser::loadXMLString($buff);
		...
		while(!feof($fp))
		{
			...
			if(trim($str) == '</attach>')
			{
				...
				if(file_exists($file_obj->file))
				{
					...
					if(Zittme\Framework\Filters\FilenameFilter::isDirectDownload($file_obj->source_filename))
					{
					...
					if(file_exists($filename))
					{
						...
						$file_obj->mime_type = Zittme\Framework\MIME::getTypeByFilename($filename);
						$file_obj->isvalid = 'Y';
