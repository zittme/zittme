...
class CommentItem extends BaseObject
{
	...
	function getContent($add_popup_menu = TRUE, $add_content_info = TRUE, $add_xe_content_class = TRUE)
	{
		...
		if($add_content_info)
		{
			...
			$content = vsprintf('<!--BeforeComment(%d,%d)--><div class="comment_%d_%d Zittme_content xe_content%s">%s</div><!--AfterComment(%d,%d)-->', array(
		...
		else
		{
			...
			{
				$content = sprintf('<div class="Zittme_content xe_content%s">%s</div>', $additional_class, $content);
			}
		}
