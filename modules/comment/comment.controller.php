...
class CommentController extends Comment
{
	...
	function procCommentVoteUpCancel()
	{
		...
		{
			throw new Zittme\Framework\Exception('failed_voted_canceled');
		}
	...
	function procCommentVoteDownCancel()
	{
		...
		{
			throw new Zittme\Framework\Exception('failed_blamed_canceled');
		}
	...
	function procCommentDeclareCancel()
	{
		...
		if (isset($comment_config->allow_declare_cancel))
		{
			...
			{
				throw new Zittme\Framework\Exception('failed_declared_cancel');
			}
		}
		else
		{
	...
	{
		if(!$manual_inserted && !Zittme\Framework\Security::checkCSRF())
		{
		...
		{
			$obj->password = \Zittme\Framework\Password::hashPassword($obj->password, \Zittme\Framework\Password::getBackwardCompatibleAlgorithm());
		}
		...
		// remove Zittme's own tags from the contents
		...
		if ($logged_info->is_admin !== 'Y')
		{
			$obj->content = Zittme\Framework\Filters\HTMLFilter::clean((string)$obj->content);
		}
	...
	function sendEmailToAdminAfterInsertComment($obj)
	{
		...
		if(isset($module_info->admin_mail) && $module_info->admin_mail && $member_info->is_admin != 'Y')
		{
			...
			// 본문 공통 부분 — 누가 어느 글에 무슨 댓글을 달았는지
			$info_rows = array(
				lang('module') => escape($browser_title),
				lang('writer') => escape($member_info->nick_name),
				lang('email_address') => escape($member_info->email_address),
				lang('document') => escape($oDocument->getTitleText()),
			);
			$comment_body = \Zittme\Framework\MailTemplate::infoTable($info_rows)
				. '<div style="padding:16px 18px;background:#f7f9fc;border-radius:12px">' . $obj->content . '</div>';
			...
			if($using_validation)
			{
				...
				$url_panel = getFullUrl('', 'module', 'admin', 'act', 'dispCommentAdminList', 'search_target', 'module', 'search_keyword', $obj->module_srl);

				// 승인제 게시판 — 승인/휴지통 링크와 대기 건수를 함께 안내한다
				$mail_content = \Zittme\Framework\MailTemplate::wrap($mail_title,
					'<p style="margin:0 0 4px">' . lang('msg_comment_waiting_approval') . '</p>'
					. $comment_body
					. '<p style="margin:20px 0 0;font-size:13px;color:#8b95a1">'
					. sprintf(lang('msg_comment_waiting_count'), $nr_comments_not_approved)
					. ' <a href="' . escape($url_panel) . '" style="color:#2677e3">' . lang('cmd_management') . '</a>'
					. '</p>'
					. '<p style="margin:10px 0 0;font-size:13px">'
					. '<a href="' . escape($url_trash) . '" style="color:#8b95a1">' . lang('cmd_trash') . '</a>'
					. '</p>',
					array('button_url' => $url_approve, 'button_label' => lang('cmd_approve'))
				);
			}
			else
			{
				$mail_content = \Zittme\Framework\MailTemplate::wrap($mail_title, $comment_body,
					array('button_url' => $url_comment, 'button_label' => lang('cmd_view'))
				);
			}
	...
	{
		if(!$manual_updated && !Zittme\Framework\Security::checkCSRF())
		{
			return new BaseObject(-1, 'msg_security_violation');
		}
		...
		// remove Zittme's wn tags from contents
		$obj->content = preg_replace('!<\!--(Before|After)(Document|Comment)\(([0-9]+),([0-9]+)\)-->!is', '', $obj->content);
		// Return error if content is empty.
	...
	function deleteComment($comment_srl, $is_admin = FALSE, $isMoveToTrash = FALSE, $childs = null)
	{
		...
		Zittme\Framework\Storage::deleteDirectory(RX_BASEDIR . sprintf('files/thumbnails/%s', getNumberingPath($comment_srl, 3)));
	...
	function moveCommentToTrash($obj, $updateComment = false)
	{
		...
		*/
		...
		// Remove the thumbnail file
		Zittme\Framework\Storage::deleteDirectory(RX_BASEDIR . sprintf('files/thumbnails/%s', getNumberingPath($obj->comment_srl, 3)));
