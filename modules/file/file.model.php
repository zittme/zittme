...
class FileModel extends File
{
	...
	public static function isDownloadable($file_info, $member_info = null)
	{
		if(!$member_info)
		{
			$member_info = Zittme\Framework\Session::getMemberInfo();
		}
	...
	public static function getUploadConfig($module_srl = 0)
	{
		...
		if (Zittme\Framework\Session::isAdmin())
		{
			// 파일 모듈 전역 설정을 한 번도 저장하지 않은 사이트에서는 null 이 돌아온다.
			$module_config = ModuleModel::getModuleConfig('file') ?: new stdClass;
			$config->allowed_filesize = max($config->allowed_filesize, $module_config->allowed_filesize ?? 0);
			$config->allowed_attach_size = max($config->allowed_attach_size, $module_config->allowed_attach_size ?? 0);
			$config->allowed_extensions = [];
