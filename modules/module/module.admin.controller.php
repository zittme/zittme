...
class ModuleAdminController extends Module
{
	...
	function procModuleAdminCopyModule($args = NULL)
	{
		...
		for($i=1;$i<=10;$i++)
		{
			...
			if(!$mid) continue;
			if(!preg_match("/^[a-zA-Z]([a-zA-Z0-9_]*)$/i", $mid)) throw new Zittme\Framework\Exception('msg_limit_mid');
	...
	function procModuleAdminInsertGrant()
	{
		...
		$module_info = $oModuleModel->getModuleInfoByModuleSrl($module_srl, $columnList);
		if(!$module_info) throw new Zittme\Framework\Exceptions\InvalidRequest;
		...
		if($module_info->module_srl)
		{
			...
			if($skin_info->extra_vars)
			{
				foreach($skin_info->extra_vars as $vars)
				{
					...
					$filename = $path . Zittme\Framework\Filters\FilenameFilter::clean($image_obj['name']);
					// Move the file
	...
	function procModuleAdminModuleSetup()
	{
		...
		if(!$vars->module_srls) throw new Zittme\Framework\Exceptions\InvalidRequest;

		$module_srls = explode(',',$vars->module_srls);
		if(count($module_srls) < 1) throw new Zittme\Framework\Exceptions\InvalidRequest;
	...
	function procModuleAdminModuleGrantSetup()
	{
		...
		if(!$module_srls) throw new Zittme\Framework\Exceptions\InvalidRequest;

		$modules = explode(',',$module_srls);
		if(count($modules) < 1) throw new Zittme\Framework\Exceptions\InvalidRequest;
	...
	function procModuleAdminInsertLang()
	{
		...
		if(!$args->name) throw new Zittme\Framework\Exceptions\InvalidRequest;
	...
	function makeCacheDefinedLangCode()
	{
		...
		foreach($lang_supported as $langCode => $langName)
		{
			...
			Zittme\Framework\Cache::set('site_and_module:user_defined_langs:0:' . $langCode, $langMap[$langCode], 0, true);
		}
	...
	public function procModuleAdminUpdateUseMobile()
	{
		...
		// 'R'(반응형)도 유효한 값이므로 공용 검증을 쓴다.
		// 예전처럼 Y가 아니면 무조건 N으로 바꾸면 반응형 설정이 조용히 사라진다.
		$moduleInfo->use_mobile = Mobile::sanitizeViewMode($useMobile);
