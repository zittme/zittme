...
class ModuleController extends Module
{
	...
	function insertActionForward($module, $type, $act, $route_regexp = null, $route_config = null, $global_route = 'N')
	{
		...
		Zittme\Framework\Cache::delete('action_forward');
		return $output;
	}
	...
	function insertTrigger($trigger_name, $module, $type, $called_method, $called_position)
	{
		...
		if($output->toBool())
		{
			...
			Zittme\Framework\Cache::delete('triggers');
		}
	...
	function insertModule($args)
	{
		...
		// New modules follow the responsive view when the site runs on it. Existing
		// modules are deliberately left alone (see the bulk apply screen), but a
		// module created after the switch should not silently start out legacy.
		if (!isset($args->use_mobile) && Mobile::isResponsiveViewEnabled())
		{
			$args->use_mobile = Mobile::VIEW_RESPONSIVE;
		}
	...
	function insertAdminId($module_srl, $admin_id, $scopes = null)
	{
		...
		{
			$args->scopes = new Zittme\Framework\Parsers\DBQuery\NullValue;
		}
		$output = executeQuery('module.insertAdminId', $args);

		Zittme\Framework\Cache::delete("site_and_module:module_admins:" . intval($module_srl));
		return $output;
	}
	...
	function _deleteModuleSkinVars($module_srl, $mode)
	{
		...
		//remove from cache
		Zittme\Framework\Cache::delete($object_key);
	...
	function insertModuleExtraVars($module_srl, $obj)
	{
		...
		Zittme\Framework\Cache::delete("site_and_module:module_extra_vars:$module_srl");
	}
	...
	function insertModuleGrants($module_srl, $obj)
	{
		...
		Zittme\Framework\Cache::delete("site_and_module:module_grants:$module_srl");
	}
	...
	function procModuleFileBoxAdd()
	{
		...
		if ($vars->filter)
		{
			...
			{
				throw new Zittme\Framework\Exception('msg_error_occured');
			}
		}
		if (in_array($ext, ['php', 'js']))
		{
			throw new Zittme\Framework\Exception(sprintf(lang('msg_filebox_invalid_extension'), $ext));
		}
		...
		{
			if(!Context::isUploaded()) throw new Zittme\Framework\Exception('msg_error_occured');
			$addfile = Context::get('addfile');
			if(!is_uploaded_file($addfile['tmp_name'])) throw new Zittme\Framework\Exception('msg_error_occured');
			if($vars->addfile['error'] != 0) throw new Zittme\Framework\Exception('msg_error_occured');
	...
	function updateModuleFileBox($vars)
	{
		...
		if($vars->addfile['tmp_name'] && is_uploaded_file($vars->addfile['tmp_name']))
		{
			...
			$random = Zittme\Framework\Security::getRandom(32, 'hex');
	...
	public function procModuleClearCache()
	{
		...
		if (!Zittme\Framework\Security::verifySignature($keystring, $signature))
		{
			throw new Zittme\Framework\Exceptions\NotPermitted;
		}
		...
		foreach ($keys as $key)
		{
			if ($key === '*')
			{
				Zittme\Framework\Cache::clearAll();
			}
			elseif (preg_match('/^([^:]+):\*$/', $key, $matches))
			{
				Zittme\Framework\Cache::clearGroup($matches[1]);
			}
			else
			{
				Zittme\Framework\Cache::delete($key);
			}
		}
	}
