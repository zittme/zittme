...
class ModuleModel extends Module
{
	...
	{
		$domain_info = Zittme\Framework\Cache::get('site_and_module:domain_info:default');
		if ($domain_info === null)
		{
			$args = new stdClass();
			...
			if ($output->data)
			{
				...
				Zittme\Framework\Cache::set('site_and_module:domain_info:default', $domain_info, 0, true);
			}
			else
			{
				$domain_info = false;
	...
	public static function getSiteInfo($domain_srl)
	{
		...
		$domain_info = Zittme\Framework\Cache::get('site_and_module:domain_info:srl:' . $domain_srl);
		if ($domain_info === null)
		{
			$args = new stdClass();
			...
			if ($output->data)
			{
				...
				$domain_info->settings = $domain_info->settings ? json_decode($domain_info->settings) : new stdClass;
				$domain_info->default_language = $domain_info->settings->language ?: config('locale.default_lang');
				Zittme\Framework\Cache::set('site_and_module:domain_info:srl:' . $domain_srl, $domain_info, 0, true);
			}
			else
			{
				$domain_info = false;
	...
	public static function getSiteInfoByDomain($domain)
	{
		...
		{
			$domain = Zittme\Framework\URL::getDomainFromURL($domain);
		}
		if (strpos($domain, 'xn--') !== false)
		{
			$domain = Zittme\Framework\URL::decodeIdna($domain);
		}
		if (strval($domain) === '')
		{
			return false;
		...
		$domain_info = Zittme\Framework\Cache::get('site_and_module:domain_info:domain:' . $domain);
		if ($domain_info === null)
		{
			$args = new stdClass();
			$args->domain = $domain;
			...
			if ($output->data)
			{
				...
				$domain_info->default_language = $domain_info->settings->language ?: config('locale.default_lang');
				Zittme\Framework\Cache::set('site_and_module:domain_info:domain:' . $domain, $domain_info, 0, true);
			}
			else
			{
				$domain_info = false;
	...
	public static function getModuleInfoByDocumentSrl($document_srl)
	{
		...
		$module_info = Zittme\Framework\Cache::get('site_and_module:document_srl:' . $document_srl);
		if (!$module_info)
		{
			$args = new stdClass();
			...
			if ($module_info)
			{
				Zittme\Framework\Cache::set('site_and_module:document_srl:' . $document_srl, $module_info, 0, true);
			}
		}
	...
	{
		// Get current domain.
		$domain = $domain ? Zittme\Framework\URL::decodeIdna($domain) : Zittme\Framework\URL::getCurrentDomain();
	...
	public static function getModuleInfoByMid($mid)
	{
		...
		$module_srl = isset(self::$_mid_map[$mid]) ? self::$_mid_map[$mid] : Zittme\Framework\Cache::get('site_and_module:module_srl:' . $mid);
		if($module_srl)
		{
			$module_info = Zittme\Framework\Cache::get('site_and_module:mid_info:' . $module_srl);
		}
		else
		{
			$module_info = null;
		...
		if($module_info === null)
		{
			...
			if($module_info)
			{
				Zittme\Framework\Cache::set('site_and_module:module_srl:' . $mid, $module_info->module_srl, 0, true);
				Zittme\Framework\Cache::set('site_and_module:mid_info:' . $module_info->module_srl, $module_info, 0, true);
			}
			else
			{
				return;
	...
	public function getModuleInfoByMenuItemSrl($menu_item_srl = 0)
	{
		...
		$module_srl = Zittme\Framework\Cache::get('site_and_module:module_srl:' . $mid);
		if($module_srl)
		{
			$mid_info = Zittme\Framework\Cache::get('site_and_module:mid_info:' . $module_srl);
		}
		else
		{
			$mid_info = null;
		...
		if($mid_info === null)
		{
			Zittme\Framework\Cache::set('site_and_module:module_srl:' . $mid, $output->data->module_srl, 0, true);
			Zittme\Framework\Cache::set('site_and_module:mid_info:' . $output->data->module_srl, $moduleInfo, 0, true);
		}
		else
		{
	...
	public static function getModuleInfoByModuleSrl($module_srl, $columnList = array())
	{
		...
		if($mid_info === null)
		{
			...
			if($mid_info)
			{
				...
				Zittme\Framework\Cache::set("site_and_module:mid_info:$module_srl", $mid_info, 0, true);
			}
		}
	...
	public static function getModulesInfo($module_srls, $columnList = array())
	{
		...
		$result = Zittme\Framework\Cache::get($cache_key);
		if ($result !== null)
		{
			return $result;
		}
		...
		Zittme\Framework\Cache::set($cache_key, $result, 0, true);
		return $result;
	}
	...
	public static function getMidList($args = null, $columnList = array())
	{
		...
		$list = $argsCount ? null : Zittme\Framework\Cache::get('site_and_module:module:mid_list');
		if($list === null)
		{
			...
			if(!$argsCount)
			{
				Zittme\Framework\Cache::set('site_and_module:module:mid_list', $list, 0, true);
			}
		}
	...
	public static function getMidByModuleSrl($module_srl)
	{
		...
		$mid = Zittme\Framework\Cache::get('site_and_module:module_srl_mid:' . $module_srl);
		if (isset($mid))
		{
		...
		if (is_object($output->data))
		{
			...
			Zittme\Framework\Cache::set('site_and_module:module_srl_mid:' . $module_srl, $mid, 0, true);
			return $mid;
		}
		else
		{
	...
	public static function getDomainByModuleSrl(int $module_srl): ?string
	{
		...
		$prefix = Zittme\Framework\Cache::get('site_and_module:module_srl_prefix:' . $module_srl);
		if (isset($prefix))
		{
		...
		if (is_object($output->data))
		{
			...
			Zittme\Framework\Cache::set('site_and_module:module_srl_prefix:' . $module_srl, $prefix, 0, true);
			return $prefix;
		}
		else
		{
	...
	{
		$action_forward = Zittme\Framework\Cache::get('action_forward');
		if($action_forward === null)
		{
			...
			Zittme\Framework\Cache::set('action_forward', $action_forward, 0, true);
		}
	...
	public static function getTriggers($trigger_name, $called_position)
	{
		...
		{
			$triggers = Zittme\Framework\Cache::get('triggers');
			if($triggers === null)
			{
				...
				if($output->toBool())
				{
					Zittme\Framework\Cache::set('triggers', $triggers, 0, true);
				}
			}
	...
	public static function getModuleInfoXml($module)
	{
		...
		$info = Zittme\Framework\Cache::get($cache_key);
		if($info === null)
		{
			$info = Zittme\Framework\Parsers\ModuleInfoParser::loadXML($xml_file);
			Zittme\Framework\Cache::set($cache_key, $info, 0, true);
		}
	...
	public static function getModuleActionXml($module)
	{
		...
		if($info === null)
		{
			$info = Zittme\Framework\Parsers\ModuleActionParser::loadXML($xml_file);
			Zittme\Framework\Cache::set($cache_key, $info, 0, true);
		}
	...
	public static function getSkins($path, $dir = 'skins')
	{
		...
		// 테마가 가진 같은 모듈의 스킨도 목록에 함께 넣는다.
		// 키가 '테마|@|스킨' 이므로 일반 스킨과 섞여도 서로 덮지 않는다.
		// (예전 XE 테마 목록을 읽던 블록이 여기 있었으나 getThemeList()가 빈 배열을
		//  돌려주는 껍데기라 동작하지 않았다. 그 자리를 대신한다.)
		$skin_list += Zittme\Framework\Theme::getModuleSkins($module, $dir);
	...
	public static function loadSkinInfo($path, $skin, $dir = 'skins')
	{
		...
		// '테마|@|스킨' 형태면 조회 기준 경로를 테마 폴더 안으로 옮긴다.
		// 모듈이든 위젯이든 원래 경로를 그대로 흉내 낸 자리에 있으므로 규칙은 같다.
		$theme_parts = Zittme\Framework\Theme::split($skin);
		if ($theme_parts)
		{
			$owner = Zittme\Framework\Theme::toRelativePath($path);
			if ($owner === null)
			{
				return;
			}
			$path = Zittme\Framework\Theme::getPath($theme_parts['theme']) . $owner . '/';
			$skin = $theme_parts['name'];
		}
		...
		$skin_info = Zittme\Framework\Parsers\SkinInfoParser::loadXML($skin_xml_file, $skin, $skin_path);
		return $skin_info;
	}
	...
	public static function getModuleConfig($module)
	{
		...
		{
			$config = Zittme\Framework\Cache::get('site_and_module:module_config:' . $module);
			if($config === null)
			{
				$args = new stdClass;
				$args->module = $module;
				...
				if($output->toBool())
				{
					Zittme\Framework\Cache::set('site_and_module:module_config:' . $module, $config, 0, true);
				}
			}
	...
	public static function getModulePartConfig($module, $module_srl)
	{
		...
		{
			$config = Zittme\Framework\Cache::get('site_and_module:module_part_config:' . $module . '_' . $module_srl);
			if($config === null)
			{
				$args = new stdClass;
				$args->module = $module;
				...
				if($output->toBool())
				{
					Zittme\Framework\Cache::set('site_and_module:module_part_config:' . $module . '_' . $module_srl, $config, 0, true);
				}
			}
	...
	public static function getModuleDefaultClass(string $module_name, ?object $module_action_info = null)
	{
		...
		{
			$namespace = 'Zittme\\Modules\\' . ucfirst($module_name);
		}
	...
	public static function isModuleAdmin($member_info, $module_srl = null)
	{
		...
		$module_admins = Zittme\Framework\Cache::get("site_and_module:module_admins:$module_srl");
		if ($module_admins === null)
		{
			...
			if ($output->toBool())
			{
				Zittme\Framework\Cache::set("site_and_module:module_admins:$module_srl", $module_admins, 0, true);
			}
		}
	...
	public static function getModuleExtraVars($list_module_srl)
	{
		...
		{
			$vars = Zittme\Framework\Cache::get("site_and_module:module_extra_vars:$module_srl");
			if($vars !== null)
			{
		...
		if(count($get_module_srls) > 0)
		{
			...
			if(!$output->data)
			{
				...
				{
					Zittme\Framework\Cache::set("site_and_module:module_extra_vars:$module_srl", new stdClass, 0, true);
			...
			foreach($output->data as $key => $val)
			{
				...
				Zittme\Framework\Cache::set('site_and_module:module_extra_vars:' . $val->module_srl, $extra_vars[$val->module_srl], 0, true);
			}
		}
	...
	{
		$skin_vars = Zittme\Framework\Cache::get("site_and_module:module_skin_vars:$module_srl");
		if($skin_vars === null)
		{
			$args = new stdClass();
			$args->module_srl = $module_srl;
			...
			Zittme\Framework\Cache::set("site_and_module:module_skin_vars:$module_srl", $skin_vars, 0, true);
		}
	...
	{
		$skin_vars = Zittme\Framework\Cache::get("site_and_module:module_mobile_skin_vars:$module_srl");
		if($skin_vars === null)
		{
			$args = new stdClass();
			$args->module_srl = $module_srl;
			...
			Zittme\Framework\Cache::set("site_and_module:module_mobile_skin_vars:$module_srl", $skin_vars, 0, true);
		}
	...
	public static function syncMobileSkinInfoToModuleInfo(&$module_info)
	{
		...
		$skin_vars = Zittme\Framework\Cache::get('site_and_module:module_mobile_skin_vars:' . $module_info->module_srl);
		if($skin_vars === null)
		{
			$args = new stdClass;
			...
			Zittme\Framework\Cache::set('site_and_module:module_mobile_skin_vars:' . $module_info->module_srl, $skin_vars, 0, true);
		}
		if(!$skin_vars) return;
	 ...
	 * @return Zittme\Modules\Module\Models\Permission
	 */
	public static function getGrant($module_info, $member_info, $xml_info = null)
	{
		...
		{
			throw new Zittme\Framework\Exceptions\DBError(lang('msg_db_query_failed'));
		}
		$grant = new Zittme\Modules\Module\Models\Permission($xml_grant_list, $module_grants->data ?: [], $module_info, $member_info ?: null);
	...
	public static function getAccessibleModuleList($member_info = null)
	{
		...
		$result = Zittme\Framework\Cache::get(sprintf('site_and_module:accessible_modules:%d', $member_info->member_srl));
		if($result === null)
		{
			...
			Zittme\Framework\Cache::set(sprintf('site_and_module:accessible_modules:%d', $member_info->member_srl), $result);
		}
	...
	{
		$output = Zittme\Framework\Cache::get("site_and_module:module_grants:$module_srl");
		if ($output === null)
		{
			$args = new stdClass;
			$args->module_srl = $module_srl;
			...
			if ($output->toBool())
			{
				Zittme\Framework\Cache::set("site_and_module:module_grants:$module_srl", $output, 0, true);
			}
		}
		return $output;
	}
