...
class Module extends ModuleObject
{
	...
	function moduleInstall()
	{
		...
		{
			$current_url = Zittme\Framework\URL::getCurrentUrl();
			$current_port = intval(parse_url($current_url, PHP_URL_PORT)) ?: null;
			$domain = new stdClass();
			$domain->domain_srl = 0;
			$domain->domain = Zittme\Framework\URL::getDomainFromURL($current_url);
	...
	function migrateDomains()
	{
		...
		if ($output->data)
		{
			foreach ($output->data as $site_info)
			{
				...
				$domain->domain = Zittme\Framework\URL::getDomainFromURL(strtolower($site_domain));
		...
		else
		{
			...
			$default_hostinfo = parse_url(Zittme\Framework\URL::getCurrentURL());

			$domain = new stdClass();
			$domain->domain_srl = 0;
			$domain->domain = Zittme\Framework\URL::decodeIdna(strtolower($default_hostinfo['host']));
			$domain->is_default_domain = 'Y';
