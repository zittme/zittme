...
class CommunicationController extends communication
{
	...
	function procCommunicationSendMessage()
	{
		...
		if(!$title)
		{
			throw new Zittme\Framework\Exception('msg_title_is_null');
		}

		$content = trim(Context::get('content'));
		if(!$content)
		{
			throw new Zittme\Framework\Exception('msg_content_is_null');
		}
		...
		{
			throw new Zittme\Framework\Exception('msg_send_mail_admin_only');
		}
		...
		if($logged_info->is_admin != 'Y')
		{
			if($receiver_member_info->allow_message == 'F')
			{
				if(!$oCommunicationModel->isFriend($receiver_member_info->member_srl))
				{
					...
					{
						throw new Zittme\Framework\Exception('msg_allow_message_to_friend');
					}
					else
					{
						throw new Zittme\Framework\Exception('msg_disallow_message');
					}
				}
			}
	...
	public function sendMessageByEmail($sender, $recipient, $title, $content): bool
	{
		...
		$mail_content = \Zittme\Framework\MailTemplate::wrap($title,
			\Zittme\Framework\MailTemplate::infoTable([
				lang('sender') => escape($sender->nick_name),
			])
			. '<div style="padding:16px 18px;background:#f7f9fc;border-radius:12px">'
			. utf8_mbencode(Zittme\Framework\Filters\HTMLFilter::clean((string)$content))
			. '</div>',
			[
				'greeting' => sprintf(lang('mail_greeting'), $recipient->nick_name),
				'button_url' => $view_url,
	...
	function procCommunicationDeleteMessages()
	{
		...
		if (!$message_srl_list)
		{
			throw new Zittme\Framework\Exception('msg_cart_is_null');
		}
	...
	function procCommunicationAddFriend()
	{
		...
		{
			throw new Zittme\Framework\Exception('msg_no_self_friend');
		}
		...
		{
			throw new Zittme\Framework\Exception('msg_already_friend');
		}
