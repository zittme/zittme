...
class TrashAdminController extends Trash
{
	...
	function procTrashAdminEmptyTrash()
	{
		...
		if(!$output->toBool()) throw new Zittme\Framework\Exception($output->message);

		if(!$this->_emptyTrash($trashSrls)) throw new Zittme\Framework\Exception($lang->fail_empty);
	...
	function _relationDataDelete($isAll, $is_type, &$trashSrls)
	{
		...
		if(is_array($output->data))
		{
			foreach($output->data as $oTrashVO)
			{
				...
				if(!is_dir(FileHandler::getRealPath($classPath))) throw new Zittme\Framework\Exception('not exist restore module directory');

				$classFile = sprintf('%s%s.admin.controller.php', $classPath, $oTrashVO->getOriginModule());
				$classFile = FileHandler::getRealPath($classFile);
				if(!file_exists($classFile)) throw new Zittme\Framework\Exception('not exist restore module class file');

				$oAdminController = getAdminController($oTrashVO->getOriginModule());
				if(!method_exists($oAdminController, 'emptyTrash')) throw new Zittme\Framework\Exception('not exist restore method in module class file');
	...
	function procTrashAdminRestore()
	{
		...
		if(is_array($trashSrlList))
		{
			...
			foreach($trashSrlList as $value)
			{
				...
				if(!method_exists($oAdminController, 'restoreTrash')) throw new Zittme\Framework\Exception('not exist restore method in module class file');
			...
			if(!$this->_emptyTrash($trashSrlList)) {
				$oDB->rollback();
				throw new Zittme\Framework\Exception($lang->fail_empty);
			}
			$oDB->commit();
		}
