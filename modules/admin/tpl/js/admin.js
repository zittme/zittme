...
// Section Toggle
	if($('.x .section').length > 1){
		...
		$section_heading.each(function(){
			var $this = $(this);
			if($this.next().length){
				$this.append('<button type="button" class="snToggle x_icon-chevron-up" aria-label="접기/펼치기"><svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M6 15l6-6 6 6"/></svg></button>');
			}
		});
