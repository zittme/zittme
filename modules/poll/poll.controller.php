...
class PollController extends Poll
{
	...
	function procPollInsert()
	{
		...
		{
			throw new Zittme\Framework\Exception('cmd_null_item');
		}
	...
	function procPollInsertItem()
	{
		...
		if($poll_item_title=='') throw new Zittme\Framework\Exception('msg_item_title_cannot_empty');

		if(!$this->user->member_srl)
		{
			throw new Zittme\Framework\Exception('msg_cannot_add_item');
		}

		if(!$poll_srl || !$poll_index_srl) throw new Zittme\Framework\Exceptions\InvalidRequest;
		...
		if(!$output->data)
		{
			throw new Zittme\Framework\Exception('poll_no_poll_or_deleted_poll');
		}
	...
	function procPollDeleteItem()
	{
		...
		if(!$this->user->member_srl)
		{
			throw new Zittme\Framework\Exception('msg_cannot_delete_item');
		}

		if(!$poll_srl || !$poll_index_srl || !$poll_item_srl) throw new Zittme\Framework\Exceptions\InvalidRequest;
		...
		if($poll_count > 0)
		{
			throw new Zittme\Framework\Exception('msg_cannot_delete_item_poll_exist');
		}
	...
	function procPoll()
	{
		...
		$output = executeQuery('poll.getPoll', $args, $columnList);
		if(!$output->data) throw new Zittme\Framework\Exception('poll_no_poll_or_deleted_poll');

		if($output->data->stop_date < date("Ymd")) throw new Zittme\Framework\Exception('msg_cannot_vote');
		...
		//if(count($tmp_item_srls)-1>(int)$output->data->checkcount) throw new Zittme\Framework\Exception('msg_exceed_max_select');
		...
		{
			throw new Zittme\Framework\Exception('msg_check_poll_item');
		}
		// Make sure is the poll has already been taken
		$oPollModel = getModel('poll');
		if($oPollModel->isPolled($poll_srl)) throw new Zittme\Framework\Exception('msg_already_poll');
	...
	function procPollViewResult()
	{
		...
		if (!Zittme\Framework\Storage::isDirectory(RX_BASEDIR . 'modules/poll/skins/' . $skin))
		{
			$skin = 'default';
		}
